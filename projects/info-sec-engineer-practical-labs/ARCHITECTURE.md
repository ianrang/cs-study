...
| 영역 | 책임 | 금지 |
|---|---|---|
| canonical 기출 데이터셋 | 기출 복원, 분석, 참고문서 매핑, 학습 전략 | 실행 스크립트, 샌드박스 산출물 |
| `projects/info-sec-engineer-practical-labs/` | 시험 대비 실습 세트, 질문, 기대 관찰값, 정리 절차 | 기출 원문 재복제, 공식 레퍼런스 레지스트리 중복 |
| 각 Lab의 `.sandbox/` | 실행 중 생성되는 일회용 파일 | git 추적, 사용자 환경 의존 |

## 2. 구조
```text
projects/info-sec-engineer-practical-labs/
├── README.md
├── ARCHITECTURE.md
├── bin/
│   ├── run-lab.sh
