...
이 문서는 `wiki/domains/information-security/drafts/study/info-sec-engineer-application-security-study.md`에서 P1으로 분류된 항목 중 **보안 속성·서버 설정·접근 정책·안전한 코드/데이터 흐름·감사와 검증 절차**를 앱의 설정형 학습 범위로 정의한다. 제품의 모든 세부 명령을 암기하는 범위가 아니라, 실기에서 반복되는 설정 빈칸·코드 판독·원리-대응 답안을 정확히 작성하는 범위다.
