...
| ID | 명제 (IF-THEN) | 예외 | 출처 |
|---|---|---|---|
| BR-P1-001 | IF 원문 P1 항목이 명령·룰·적용 위치·통제 선택·운영 정책을 직접 제시 THEN 해당 항목은 최소 한 문제로 표현한다. | 단순 조회 명령·순수 개념·P2/P3은 제외한다. | 사용자 요청, `wiki/domains/information-security/drafts/study/info-sec-engineer-network-security-study.md:24,54` |
| BR-P1-002 | IF 답이 하나의 문자열 또는 배열 순서로 결정 THEN 기존 short/cloze/order handler로 자동 채점한다. | 복수 정상 운영안·제품/버전 의존 답은 essay handler로 둔다. | PRD FR-3·4, `wiki/domains/information-security/drafts/study/info-sec-engineer-network-security-study.md:578,635,683` |
...
| BR-P1-006 | IF 원문은 하나의 보완책을 다른 보완책과 병행하라고 명시 THEN 단일 명령 암기가 아닌 병행 관계 또는 적용 한계를 해설/rubric에 포함한다. | 결정적 명령 자체를 묻는 빈칸은 유지한다. | `wiki/domains/information-security/drafts/study/info-sec-engineer-network-security-study.md:215,225,511,617` |
