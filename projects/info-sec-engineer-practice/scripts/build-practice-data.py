...
from past_exam_converter import build_past_exam_payload, validate_past_exam_payload
...
PAST_EXAM_OUTPUT_PATH = DATA_ROOT / "generated" / "past-exams.json"
...
def validate_source_ref(ref: object, question_id: str, errors: list[str]) -> None:
    ...
    if not isinstance(path, str) or not path:
        ...
        return
    source_path = KNOWLEDGE_ROOT / path
...
def build_payload(
    curriculum: dict[str, object], packs: list[dict[str, object]], past_exams: dict[str, object]
) -> dict[str, object]:
    questions = [question for pack in packs for question in pack["questions"]]
    return {"version": 1, "curriculum": curriculum, "questions": questions, "pastExams": past_exams}
...
def main() -> int:
    ...
    try:
        ...
        past_exams = build_past_exam_payload(KNOWLEDGE_ROOT)
    ...
    errors.extend(validate_past_exam_payload(past_exams))
    if errors:
        print("question bank validation failed:", file=sys.stderr)
        print("\n".join(f"- {error}" for error in errors), file=sys.stderr)
        return 1

    payload = build_payload(curriculum, packs, past_exams)
    ...
    generated_past_exams = json.dumps(past_exams, ensure_ascii=False, indent=2) + "\n"
    if args.check:
        if (
            not OUTPUT_PATH.is_file()
            or OUTPUT_PATH.read_text(encoding="utf-8") != generated
            or not PAST_EXAM_OUTPUT_PATH.is_file()
            or PAST_EXAM_OUTPUT_PATH.read_text(encoding="utf-8") != generated_past_exams
        ):
            print("generated practice data is missing or stale; run python3 scripts/build-practice-data.py", file=sys.stderr)
            return 1
        print("question bank validation passed; generated data is current")
        return 0
    PAST_EXAM_OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    PAST_EXAM_OUTPUT_PATH.write_text(generated_past_exams, encoding="utf-8")
    OUTPUT_PATH.write_text(generated, encoding="utf-8")
    print(
        f"validated {len(payload['questions'])} learning questions and "
        f"{sum(len(round_data['items']) for round_data in past_exams['rounds'])} past-exam items"
    )
    return 0
