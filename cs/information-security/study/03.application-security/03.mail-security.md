...
| 회사 메일 서버 | **MTA** 가 *대외 SMTP 전달*의 핵 — 한 호스트에 **제출(MSA)·저장(MDA)·IMAP/POP(MRA)** 이 같이 붙는 구성이 일반적 |
| 메일이 늦게 도착할 때 큐에 쌓인 그것 | **메일 큐** — 대표 경로로 Sendmail 계열 **`/var/spool/mqueue`** *(Postfix 등은 `/var/spool/postfix` 등 **구현체별**)* |
| 휴대폰·노트북 모두에서 같은 메일 | IMAP (서버 사본 유지) |
| "외부에서 발송된 메일" 노란 배너 | 보통 **수신 게이트웨이·웹메일의 ‘외부 발신자’ 표시**(로컬 정책). **DMARC `p=`**(DNS)는 *인증 실패 시 처리* 정책 — **서로 다른 개념** |
| 도메인 DNS TXT 의 `v=spf1` | SPF |
| 헤더의 `DKIM-Signature` | DKIM |
| 헤더의 `Authentication-Results` (`spf=`·`dkim=`·`dmarc=` …) | 수신 측 **SPF·DKIM·DMARC 등** 인증 결과 (RFC 8601 계열 관행) |
| 결제·계약서 **PDF 파일** 자체의 디지털 서명 | **PDF 서명**(PDF 객체·PKCS#7 in PDF 등) — **S/MIME** 은 *MIME 메일 본문·파트* 서명·암호화 |
| SMTP **TLS** 로 제출·전달 구간 보호 | **SMTPS 465/tcp**(implicit TLS) 또는 **587/tcp 제출 + STARTTLS** — 링크 스킴은 **`mailto:`**(RFC 6068) 가 표준; **`mailtos://`** 는 시험·표준에서 외우지 말 것 |
...
[1] 송신자가 MUA 로 메일 작성
[2] MUA → MTA: SMTP 로 전송
[3] MTA: 메일 큐에 임시 저장 *(Sendmail 계열: `/var/spool/mqueue` · Postfix 등은 별도 큐 디렉터리)*
...
- "POP3 = ?" → 이동 (110/tcp).
- "IMAP = ?" → 복사 (143/tcp).
- "메일 큐 디렉터리 *대표 예*?" → Sendmail **`/var/spool/mqueue`** · Postfix 는 **`/var/spool/postfix`** 등 *구현체별*.
...
sequenceDiagram
    ...
    Note over SM: [3] 메일 큐 임시 저장 (그림: Sendmail 예 /var/spool/mqueue)
    SM->>DNS: MX 레코드 질의
    DNS-->>SM: 수신 MTA 주소
    SM->>RM: [4] SMTP 재전송
    RM->>MD: [5] 사용자 메일함 저장
...
큐·메일함     — mqueue(Sendmail) · /var/spool/mail (Postfix 큐는 /var/spool/postfix 등)
로그          — /var/log/maillog
...
5. 메일 큐 디렉터리 *대표 경로*? (Sendmail vs Postfix)
6. 사용자 메일함 디렉터리 경로?
...
| "회사 메일 서버" = MTA 만? | MTA 가 전달의 *핵*이나 실제는 **MSA·MDA·MRA(IMAP/POP)** 가 한 시스템에 공존 |
| 메일 큐 경로 = 항상 mqueue? | **Sendmail** `/var/spool/mqueue` · **Postfix** 등은 `/var/spool/postfix` 등 *구현체별* |
| 노란 "외부 발송" 배너 = DMARC `p=`? | **아님** — 보통 **로컬 게이트웨이·웹메일 태깅**. DMARC `p=` 는 **DNS 정책**(인증 실패 시 수신 처리) |
| PDF 파일 서명 = S/MIME? | **아님** — PDF 는 PDF 서명 체계; **S/MIME** 은 *MIME 메일* 서명·암호화 |
| `mailtos://` = SMTPS 암기? | **표준 없음** — **`mailto:`**(RFC 6068). TLS 는 **465/587·STARTTLS** 로 외운다 |
