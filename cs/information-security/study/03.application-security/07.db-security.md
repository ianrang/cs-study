# 07. 데이터베이스 보안 — 만화책처럼 읽는 ACID 와 DB 보안 5대 축

> **이 문서를 읽는 법**: 아래 *5 가족*(트랜잭션·SQL·위협·통제·암호화)은 **시험·복습용 학습 분류**로, 엄밀한 표준 용어의 ‘통제 목록’과 1:1로 대응하지는 않는다 — *ACID·SQL 4분류는 DB 보안을 이해하기 위한 기반 개념*을 같은 줄에 묶어 둔 것이다. 05 단원 **SQL Injection** 은 *입력 검증 실패*로 요약할 수 있으나, 실무·시험 확장 관점에서는 **동적 SQL 문자열 결합**, **Prepared Statement(파라미터 바인딩) 미사용**, **DB·스키마 최소 권한 미적용** 등과도 연결된다. 본 단원의 집성·추론처럼 *DB 메커니즘·통제* 축이 자연스럽게 보이도록 구성했다.
...
| 매일 보는 것 | 사실은 이 단원의 |
|---|---|
| 트랜잭션 `BEGIN` … `COMMIT` 한 묶음 | *시작* 문법은 DBMS별 상이 — 시험·표준 심화는 **TCL = `COMMIT`·`ROLLBACK`** 중심 (§2) |
| 결제 실패 시 자동 롤백 | ACID 의 **원자성** (§1) |
| `GRANT SELECT ON ... TO ...` | DCL + SQL 기반 접근 통제 (§4·§5) |
| 일반 사용자가 `salary` 컬럼 안 보임 | 뷰 + **DCL (`GRANT`)** 로 컬럼 은닉 (§4·§5) |
| "30대 남성 평균 연봉" 통계로 1명 노출 | **추론 (Inference) 위협** (§3) |
| 카드번호·주민번호 **컬럼** 암호화 | **컬럼** 암호화·API / Plug-In (§9) — *디스크 전체 TDE(§10)와 구분* |
| 디스크 도난 대비 전체 암호화 | **TDE** (Transparent Data Encryption, §10) |
| MySQL `skip-networking` 옵션 | **TCP/IP 리스너 비활성화**(방화벽과 별개) → **3306/tcp 가 열리지 않음**(§13) |
| `WITH GRANT OPTION` 의 의미 | 권한 위임 체인 (§5) |

> **이 단원의 본질**: 05 단원이 *어플리케이션 코드* 라면 07 단원은 *DB 엔진의 메커니즘*. **트랜잭션 (ACID) + SQL 분류 (DML·DDL·DCL·TCL) + 위협 (집성·추론) + 통제 (접근·추론·흐름) + 암호화 (컬럼·블록)** 을 한 줄에 묶은 *5 가족*이 핵심(위 문단처럼 *학습용 축*).
...
| **TCL** | **T**ransaction **C**ontrol Language | 트랜잭션 *작업 단위 제어* | **`COMMIT`·`ROLLBACK`** (심화: `SAVEPOINT` 등) |

### 장면. TCL 과 트랜잭션 *시작* 문법

트랜잭션 *시작*은 `BEGIN`, `START TRANSACTION` 등 **DBMS·버전별로 다르다**. 시험에서는 **종료·취소**인 **`COMMIT`·`ROLLBACK`** 을 TCL 의 대표로 외우면 안전하다.
...
- "`CREATE` 의 분류?" → **DDL**.
- "`SELECT` 의 분류?" → **DML**.
- "`BEGIN` 이 TCL 의 전부?" → *시작* 문법은 DBMS별 — 시험 대표는 **`COMMIT`·`ROLLBACK`**.
...
> 05 단원의 *SQL Injection* 은 *신뢰할 수 없는 입력이 쿼리에 섞이는* 문제로, *입력 검증 실패*로 요약되기도 하지만 **동적 SQL 문자열 결합**, **Prepared Statement 미사용**, **과도한 DB 권한** 과 함께 이해하는 것이 정확하다. **본 단원의 집성·추론은 *DB 자체의 메커니즘*에서 발생** — 사용자가 *정당하게 권한이 있어도* 발생 가능한 위협.
...
> *기본 테이블에 직접 접근을 차단*하면서 *허용한 일부 컬럼만 노출* — 보안과 사용성의 균형. 일부 DBMS 는 **컬럼 단위 `GRANT`** 로도 유사한 은닉이 가능하나, 시험·교재에서는 **뷰 + DCL** 조합이 자주 나온다.

### 시험 함정

- "뷰의 정체?" → *가상의 테이블* (실제 존재 ❌).
- "뷰의 보안 가치?" → *컬럼 은닉* (실무 패턴: **뷰에만 `GRANT`**).
...
- DBMS 엔진에 *최적화된 성능*
- **저장소·파일 도난** 등 *디스크 상 데이터* 보호에 유효하지만, **정상 계정으로의 SQL 조회** 시에는 엔진이 복호화한 **평문이 그대로 보일 수 있다** → *SQL Injection·권한 탈취·내부자 악용*을 **TDE 만으로 막는 것은 아니다**(파라미터 바인딩·최소 권한·접근 통제가 별도 계층).
...
- "TDE 만으로 SQL Injection 도 막나?" → ❌ — *저장 암호화*와 *앱/권한 계층*은 별개.
...
| **원격 접속 차단** | `my.cnf` 의 **`skip-networking`** → mysqld 가 **TCP/IP 리스너를 열지 않음**(방화벽 규칙과는 별 메커니즘). 로컬은 **`mysql.sock`** 등으로 동작 |
...
> **`skip-networking`**: MySQL 서버의 **TCP/IP 네트워킹을 끈다**(공식 문서의 `--skip-networking` / `skip_networking` 과 동일 취지). **결과적으로 3306/tcp 리스닝이 없어지며**, OS 방화벽으로 포트를 막는 것과는 **다른 레이어**의 설정이다.
...
> **시험 함정**: `skip-networking` 은 *로컬은 유닉스 도메인 소켓 등으로 유지*하면서 **TCP/IP 접속만 불가**하게 한다. **완전한 mysql 비활성화 ❌**.

### 장면 4. 운영 패턴

| 환경 | 옵션 |
|---|---|
| 같은 서버에 어플리케이션 + DB | `skip-networking` 적용 가능 (TCP/IP로 원격 접속 불가) |
| 별도 서버에서 DB 접속 | `bind-address` 로 특정 IP 만 허용 |
| 외부 노출 필요 | 방화벽 + TLS + 사용자 호스트 제한 |

### 시험 함정

- "`skip-networking` 의 효과?" → **TCP/IP 리스너 비활성화**(3306/tcp 미오픈) + *로컬(소켓) 통신 유지*.
...
graph TD
  ...
  Q1 -- 운영 --> OPS[MySQL 점검]
  OPS --> OPS1[root 계정 변경]
  OPS --> OPS2[skip-networking → TCP/IP 리스너 끔]
...
DCL — GRANT · REVOKE
TCL — COMMIT · ROLLBACK (심화: SAVEPOINT) · 시작은 BEGIN 등 제품별
```
...
포트    — skip-networking 시 TCP/IP 리스닝 없음(3306 기본 미오픈)
...
| TDE 만으로 SQL Injection 방어? | ❌ (*저장 암호화* ≠ 앱/권한 계층) |
| 파일 암호화의 *추가 적용*? | 비정형 데이터 (로그·이미지·음성·영상) |
| MySQL 포트? | **3306/tcp** |
| 원격 차단 옵션? | **`skip-networking`** |
| `skip-networking` 의 효과? | **TCP/IP 리스너 비활성화**(3306/tcp 미오픈) + *로컬 소켓 유지* |
| 로컬 통신 수단? | **`mysql.sock`** |
| 위험한 3 권한? | **File · Process · Shutdown** |
| mysql 계정 로그인 쉘? | `/bin/false` 또는 `/sbin/nologin` |
