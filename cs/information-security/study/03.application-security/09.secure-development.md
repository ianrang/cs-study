# 09. 어플리케이션 개발 보안 — 만화책처럼 읽는 KISA 7 가족과 5 도구

> **이 문서를 읽는 법**: 05 단원의 21 공격을 *코드 레벨에서 어떻게 예방할까*. KISA 7 카테고리로 약점을 *분류*하고, 5 도구로 *시점별로 검출*하는 흐름을 시간 순으로 따라가면 모든 시험 함정이 풀립니다.
...
| 매일 보는 것 | 사실은 이 단원의 |
|---|---|
| CI 의 SonarQube 단계 | **SAST** (정적 코드 분석) |
| 로컬 OWASP ZAP·Burp Suite 점검 | **DAST** (동적 분석) |
| `npm audit` / Dependabot PR | **SCA** (오픈소스 CVE 매칭) |
| 코드 리뷰 시 `gets()`·`strcpy()` 지적 | KISA 7대 ① 입력 / ⑦ API 오용 |
| `password = "admin123"` 하드코딩 발견 | KISA 7대 ② 보안 기능 (CWE-798) |
| Threat Model 회의에서 STRIDE | §1 위협 모델링 |
| 빌드 실패 → "Test before Deploy" | Shift-Left Security |
| 운영 환경의 Imperva·Contrast | **RASP** (런타임 자체 방어) |
| Log4Shell 패치 비상 대응 | A06 + SCA 영역 |

> **이 단원의 본질**: 05 단원이 *공격*이라면 09 단원은 *예방*. **Shift-Left** = 보안을 *개발 흐름의 왼쪽 (앞단)* 으로. **KISA 7 카테고리**로 약점을 분류하고, **5 도구 (SAST·DAST·IAST·SCA·RASP)** 로 *시점별로 검출*. *분류와 도구의 매칭*이 핵심.
...
| **A06** Vulnerable and Outdated Components | API 오용 — *SCA 도구 영역* |
...
- "A06 Vulnerable Components 의 KISA·도구?" → API 오용 / **SCA**.
...
**RASP 대표 도구**: Imperva RASP, Contrast Protect, Fortify Application Defender.
...
KISA 7 카테고리 + 5 도구의 *시점별 매핑*.

| KISA 카테고리 | 1차 도구 | 시점 |
|---|---|---|
| 1 입력 데이터 검증 및 표현 | SAST + DAST | 구현 + 테스트 |
| 2 보안 기능 | SAST + 코드 리뷰 | 구현 |
| 3 시간 및 상태 | SAST (Race Condition) + IAST | 구현 + 테스트 |
| 4 에러 처리 | DAST | 테스트 |
| 5 코드 오류 | SAST | 구현 |
| 6 캡슐화 | SAST + 코드 리뷰 | 구현 |
| 7 API 오용 | SAST + SCA | 구현 + 빌드 |

> **읽는 법**: *KISA 카테고리별 적합 도구가 다르다*. 카테고리 7 (API 오용) 은 *SAST + SCA* 를 함께 — 자체 코드 + 외부 라이브러리 둘 다 점검.
...
```
SAST — 정적 (코드) — White-box — SonarQube · Checkmarx · Fortify · Coverity · Semgrep
DAST — 동적 (실행) — Black-box — OWASP ZAP · Burp Suite · Acunetix · Netsparker
...
RASP — 런타임 자체 방어 — Imperva RASP · Contrast Protect · Fortify Application Defender
```
...
18. A06 Vulnerable Components 의 KISA·도구?
