...
> **이 문서를 읽는 법**: 21 개 공격을 *5 가족*(인젝션·접근통제·인증·설정·암호화) 으로 묶고, 각 공격이 *어디서 발생하고*(클라이언트 / 웹서버 / DB) *어떤 방어로 막히는지*를 시간 순으로 따라가면 외울 게 정리됩니다.
...
| `escape()` / `htmlentities()` | XSS 의 *표준 방어* (§3) |
| 폼 제출 시 hidden 필드의 `_csrf` 토큰 | CSRF 토큰 (§4) |
| `<img src="https://...">` 자동 GET 발생 | CSRF 의 *공격 매개체* (§4) |
...
> **이 단원의 본질**: 21 공격은 *공격자가 누구의 손을 빌려 무엇을 깨는가*로 가족을 이룬다. *공격자 → 희생자 (XSS·CSRF) / 공격자 → 서버 (SQLi·OS Cmd·SSRF) / 공격자 → 인증 (세션·인가)* — 한 줄 분류로 21 가지가 정리된다.
...
| **입력값 치환 (HTML Entity)** | HTML 코드로 인식될 수 있는 특수문자를 *HTML Entity 로 치환* |
...
- "XSS 의 표준 방어?" → **HTML Entity 치환** (출력 인코딩).
...
CSRF 취약점 판단은 입력 가능한 폼에 **`<img>` 태그의 `src` 속성**을 사용해 조작된 요청을 입력하여 진행 — *게시글을 읽기만 해도 이미지 태그가 자동 요청*을 발생시키기 때문.
...
> **URL/파라미터 변조 취약점**: 실행 경로에 대해 접근제어를 검사하지 않거나 불완전 구현하여 공격자가 *URL/파라미터값을 조작*하여 중요 정보에 접근.
...
> - XSS 표준 방어 = *출력 인코딩 (HTML Entity 치환)* (입력값 차단 ❌)
> - 모범 사례: **입력 시점이 아닌 출력 시점에 이스케이프**.
...
| 2 | XSS 표준 방어? | **HTML Entity 치환** (출력 인코딩) |
...
graph TD
  ...
  D3 --> A3[XSS 출력 인코딩 · 파일 업로드 확장자<br/>SSRF · OS Command]
...
**읽는 법**: 입력값 검증 (D1) 은 *1차 방어이지만 충분하지 않다*. SQLi 는 *D2 (선처리 질의문)*, XSS 는 *D3 (출력 인코딩)* 가 표준.
...
15. XSS 의 표준 방어와 HTML Entity 9종 중 `<`·`>`?
16. CSRF 의 *4 대응*?
...
| XSS 표준 방어? | **HTML Entity 치환** |
| CSRF 발생 주체? | *희생자* |
| SSRF 발생 주체? | *웹서버* |
| OWASP A10 단독 신규? | SSRF |
| `<img>` 태그가 무서운 이유? | 게시글 *읽기만 해도 자동 요청* |
| OS Command 필터링 문자? | `;`·`&&`·`||`·`|` |
