...
| 매일 보는 것 | 사실은 이 단원의 |
|---|---|
| 브라우저에 `example.com` 입력 → 페이지 로딩 | DNS 재귀적/반복적 질의 — **주로 53/udp**, 응답이 크거나 **존 전송(AXFR 등)은 53/tcp** (§에피소드 1 표 참고) |
| `/etc/hosts` 에 추가한 한 줄 | 로컬 매핑 — **많은 환경·시험 해설에서는** DNS 네임서버보다 hosts 가 먼저 조회되지만, **OS 의 이름 해석 순서 설정에 따라 달라짐** (§윈도우 우선순위·`/etc/nsswitch.conf` 등) |
...
| `public` / `private` Community String | SNMP v1·v2c 용 **Community String** *(공유 문자열·약한 인증/식별, 평문 노출)* |

> **이 단원의 본질**: 시험은 *인프라 동작*과 *보안 위협 — 그리고 위협이 어디서 왔는지*만 묻는다. **전통적인** DNS·DHCP·SNMP(v1·v2c) 는 *암호·강인 인증이 빠진 채 표준화*되어 **평문·약인증 경로가 많고**, 위협 패턴이 닮아 있다. 다만 같은 묶음 안에서도 **DNSSEC·DoT/DoH**, **SNMPv3(USM)** 등 **후속 표준으로 보완**할 수 있다는 점까지 함께 본다.
...
| DNS 의 핵심 특성 | 의미 |
|---|---|
| **비연결** | *일반 조회는* **UDP** 중심으로 비연결적이나, 큰 응답·존 전송 등에는 **TCP** 도 사용한다 (RFC 1035 등) |
...
> *(역할 정리)* **재귀**는 보통 **클라이언트 ↔ 재귀 네임서버** 한 번의 “끝까지 찾아줘” 요청이고, 그 재귀 서버가 **루트 → TLD → 권한 DNS** 에게 차례로 물으며 최종 IP 를 모으는 과정이 **반복적 질의**에 해당한다.
...
`hosts.ics` 는 인터넷 연결 공유 (ICS, Internet Connection Sharing) 사용 시 클라이언트 IP 를 강제 지정하는 기능에 쓴다. 리눅스·유닉스에서는 **`/etc/nsswitch.conf` 의 `hosts:` 항목 순서**(구형 환경의 **`/etc/host.conf`** `order`) 로 이름 해석 순서가 결정된다.

> **시험 핵심**: "**많은 환경에서 hosts 가 네임서버 DNS 조회보다 앞선다**" — 파밍 대응으로 중요 도메인을 hosts 에 고정하는 근거가 될 수 있다. *(단, 실제 순서는 OS 설정·nsswitch 등에 따르므로 절대 규칙으로 외우지 말 것.)*
...
| `/etc/hosts` | 도메인/호스트명 ↔ IP 매핑 (**많은 기본 설정에서** 네임서버보다 앞서 참조될 수 있음 — **`/etc/nsswitch.conf` 등으로 순서 변경**) |
| `/etc/host.conf` | 이름 해석 순서 지정 *(구형; 리눅스 계열은 보통 **`/etc/nsswitch.conf`** 가 우선)* |

### 장면 4. hosts 파일이 무기가 되다 — 파밍 (Pharming)

`/etc/hosts` 가 네임서버 조회보다 *먼저 적용되는 구성이 흔하다*. *악성코드가 hosts 를 변조하면* 정상 도메인을 입력해도 공격자 사이트로 향할 수 있다 — **파밍 (Pharming)** 공격의 근거.
...
- "공격이 작동하는 *DNS 본성 2가지*?" → *비연결 조회(UDP·53 중심)* + 먼저 받은 응답 신뢰. *(일부 응답은 TCP 이나, 스푸핑 논점에서는 이 둘이 핵심.)*
...
> *(포트 혼동 방지)* 서버는 **67** 에서 대기하고 클라이언트는 **68** 에서 대기한다. Discover 등 클라이언트→서버 패킷의 **목적지**는 보통 **67**, 서버→클라이언트 응답의 **목적지**는 **68** 이다.
...
> **Community String**: SNMP 데몬과 관리자가 데이터 교환 전에 맞추는 **공유 문자열**로, v1·v2c 에서 사실상 **약한 인증·식별** 역할을 한다. **평문에 가깝게 노출**되므로 스니핑에 취약하다 — 강인 인증은 **SNMPv3** 가 담당한다.
...
| hosts 파일과 DNS 의 우선순위? | **OS 이름 해석 순서 따름**. 시험·실무에서는 hosts 가 DNS 보다 앞선 경우가 많다고 외운다 |
...
| Manager·Agent 포트? | Agent **161/udp** (Get/Set 수신) · Manager **162/udp** (Trap 수신) |
...
> **다음 단원**: [[cs/information-security/03.application-security/02.ftp-security|02. FTP 보안]] — 본 단원 §1 의 *전통적 인프라의 평문·약인증·(DNS 는 조회의) 비연결성* 패턴이 02 단원의 *제어 채널·데이터 채널 분리*와 직접 연결.
