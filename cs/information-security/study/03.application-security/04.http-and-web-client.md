...
| 매일 보는 것 | 사실은 이 단원의 |
|---|---|
| `fetch('/api/...')` 의 응답 헤더 | HTTP 응답 **4영역 중 헤더 영역**(상태라인·빈 줄·바디와 구분) |
| `Set-Cookie: session=...; HttpOnly; Secure` | 쿠키 보안 속성 일부 — HttpOnly·Secure·SameSite·Expires/Max-Age·Domain·Path 구분 (§11·§12) |
| 로그인 후 다음 페이지에서도 인식되는 사용자 | 세션 + 세션 ID |
| 로그아웃 안 했는데 다른 PC 에서 자동 로그인 됨 | **브라우저 프로필·쿠키 동기화**, **SSO**, **Remember-me·리프레시 토큰** 등 *(영속 쿠키는 주로 **같은 기기·같은 브라우저**에서 브라우저를 닫아도 유지)* |
| 프론트에서 쿠키 못 읽을 때 | `HttpOnly` 적용 |
| HTTP 로 접속해도 HTTPS 로 **강제 전환**되는 경우 | **HSTS** (이미 정책을 저장한 뒤). **최초 HTTP 접속**·preload 미적용 시 공백은 §13 |
| `401 Unauthorized` vs `403 Forbidden` | **인증** 부족·무효 vs 요청은 이해되나 **수행 거부**(권한·정책 등) |
| 브라우저가 cross-origin 요청 전 보내는 **`OPTIONS` 예비 요청** | **CORS Preflight** *(모든 API 의 OPTIONS 응답이 CORS 는 아님 — §7)* |
| `curl -I` 가 보내는 메소드 | HEAD |
| `Content-Type: application/json` | **요청·응답** 메시지 모두에 올 수 있는 표현 미디어 타입 헤더 |

> **이 단원의 본질**: HTTP 는 *원래 무상태 (Stateless)*. 그 한계 때문에 쿠키·세션이 태어났고, *그 쿠키가 웹 공격의 핵심 표적*이 되어 *보안·수명 속성 6종* (HttpOnly·Secure·Expires/Max-Age·SameSite·Domain·Path) 이 정리됐다. 무상태 → 쿠키 → 보안 속성 → 다음 단원의 공격 — *한 줄 인과*로 외운다.
...
| 헤더 | 용도 |
|---|---|
| `Content-Type` | 메시지 바디의 *데이터 형식* (MIME 타입). **요청·응답** 양쪽에 사용 (예: POST JSON) |
...
| **401 Unauthorized** | *인증* 이 없거나 무효·부족 — **유효한 자격 증명**이 필요 (로그인·토큰 갱신 등으로 풀리는 경우가 많음) |
| **403 Forbidden** | 서버가 요청을 *이해했으나* **수행 거부** — 인증은 있어도 정책·권한·명시적 차단 |
| **404 Not Found** | 자원 *존재하지 않음* |

### 장면 4. 한 줄 외우기

> **401** = *인증* 문제 — *유효한 자격 증명*을 내면 풀리는 경우가 많다. **403** = *인가·정책* 등으로 **거부** — 로그인했어도 막힐 수 있음.

### 시험 함정

- "401 vs 403?" → 인증(자격 증명) 부족·무효 vs 이해 후 수행 거부(권한·정책).
- "301 vs 302?" → 영구 vs 임시.
...
| 종류 | 저장 위치 | 수명 |
|---|---|---|
| **영속 쿠키 (Persistent Cookie)** | 클라이언트의 **파일** | `Expires` 또는 `Max-Age` 로 수명 지정 (§11) |
...
| **Expires** / **Max-Age** | *절대 만료 시각* / *상대 수명(초)* — 둘 다 있으면 **Max-Age 우선**(RFC 6265) | 길면 노출 위험 ↑ → *최소한*으로 |
...
| Expires·Max-Age 가 과도하게 김 | *1차 노출이 장기·사실상 영구 노출*로 이어짐 |
...
- "Expires 와 Max-Age 를 둘 다 줬을 때?" → **Max-Age 우선**(RFC 6265).
...
| Expires / Max-Age | 유효기간·수명(초) | 장기 노출 |
