...
> **이 문서를 읽는 법**: 카드 결제가 *왜 단순 SSL 만으로 부족했는지*, *SET 의 이중 서명이 어떤 분리를 만들었는지*, *왜 3DS 2 가 비밀번호를 버렸는지* — 시간 순으로 따라가면 모든 시험 함정이 풀립니다.
...
| 매일 보는 것 | 사실은 이 단원의 |
|---|---|
| HTTPS 결제 페이지 | SSL/TLS 기반 결제 (§3) |
| 카드 결제 시 별도 비밀번호 | 3DS 1.0 (Verified by Visa·MasterCard SecureCode) |
| 결제 시 자동 OTP 알림 | 3DS 2 의 도전 인증 (Challenge) |
| 일부 결제는 비밀번호 입력 없이 통과 | 3DS 2 의 마찰 없는 인증 (Frictionless) |
| 카카오페이·네이버페이 간편 결제 | 전자지갑 (§1) |
| Apple Pay·Google Pay 비접촉 결제 | NFC + 토큰화 (§12) |
| 인터넷뱅킹 OTP 토큰 | TOTP·HOTP (§9) |
| 소셜 로그인 (Google·Naver·Kakao) | OpenID Connect (§10) |
| 사설 인증서로 금융 거래 | 2020.12 시행 전자서명법 개정 후 |
...
## 📖 에피소드 7 — 3DS 1.0 vs 3DS 2: 비밀번호에서 위험 기반으로
...
| 사용자 경험 | *매 거래마다* 비밀번호 (구매 포기율 ↑) | 저위험 거래는 **마찰 없는 인증 (Frictionless Authentication)** — 사용자 입력 ❌ |
...
| **3DS 1.0** | 2001 | 카드 소지자 추가 인증 (비밀번호) | 2022 deprecated |
| **3DS 2** | 2017 | *위험 기반* 인증 (Frictionless / Challenge) | *현재 표준* |

> **읽는 법**: SSL/TLS 가 *표준*이지만 카드 정보 도용 위험은 남는다 → *3DS 가 카드 소지자 인증 추가*. 3DS 1.0 은 마찰이 컸기에 → *3DS 2 의 위험 기반 인증* 으로 진화.
