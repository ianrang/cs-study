...
| 매일 보는 것 | 사실은 이 단원의 |
|---|---|
| `scp file user@server:/path/` | SFTP 의 친척 (SSH-2 기반, 22/tcp) |
...
| `/etc/hosts.allow` · `hosts.deny` | TCP Wrapper — vsFTP 와 연동 |
...
> **TFTP (Trivial File Transfer Protocol)**: 읽기 전용 메모리나 디스크가 없는 워크스테이션에 설치될 수 있을 정도로 *매우 단순*한 파일 송수신 프로토콜. **69/udp** 사용.
...
- "TFTP 의 포트?" → **69/udp** (FTP 21/tcp 와 *전송 계층도 다르다*).
...
운영체제 단의 `hosts.allow`·`hosts.deny` 파일을 통한 *IP 화이트/블랙리스트*가 그대로 vsFTP 에 적용 ([[cs/information-security/study/01.system-security/06.system-attacks|정확본 01-06 TCP Wrapper]] cross-ref).
...
> **시험 핵심**: "SFTP = SSH 22/tcp" / "FTPS = SSL/TLS 990 또는 21/tcp" — 이름이 비슷해서 혼동하기 쉬운 *함정 1순위*.
...
| TFTP 의 포트·전송계층? | **69/udp** (FTP 와 다름) |
