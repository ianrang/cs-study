...
> **연결 단원**: 본 단원의 *Ping of Death·Smurf*는 [[cs/information-security/02.network-security/02.network-protocols-and-addressing|02 단원]] 의 *ICMP·IP 단편화*가 토대. *TCP SYN Flooding*은 02 단원의 *3-way Handshake* 악용. *IP Spoofing 출발지 위조*는 [[cs/information-security/02.network-security/04.scanning-sniffing-spoofing|04 단원]] 의 *IP Spoofing*. *DNS 싱크홀*은 봇넷 *C&C* 차단 기법. *Tiny Fragment / Fragment Overlap*은 *DoS 가 아닌 패킷 필터링 우회 공격*.
...
| *Smurf Attack* | **Directed Broadcast + ICMP Echo Reply 증폭** |
...
> **이 단원의 본질**: 모든 공격이 *CIA 의 **가용성 (Availability)**을 표적*. 분류 축은 (1) *공격자 수* (단일 vs 분산), (2) *직접 vs 반사·증폭*. **DDoS = 봇넷 직접 / DRDoS = 반사서버 경유 + IP 위조 필수 + 증폭 필수**. *Tiny Fragment·Fragment Overlap 은 DoS 가 아닌 우회 공격* — 시험 함정.
...
| 구분 | 풀이 | 공격자 수 | 직접/간접 | 위조 IP | 증폭 |
|---|---|---|---|---|---|
| **DoS** | **D**enial **o**f **S**ervice | **1** | 직접 | 일부 사용 | 일부 (Smurf 등) |
| **DDoS** | **D**istributed **D**oS | *다수 (봇넷)* | 직접 | 일부 사용 | 거의 없음 |
| **DRDoS** | **D**istributed **R**eflection **D**oS | *다수 (봇넷)* | **반사서버 경유** | **반드시 사용** | **반드시 발생** |

### 외우는 방법 — *D 가 늘수록 분산·반사*

> *DoS (1 자)* / *DDoS (분산)* / *DRDoS (분산 + 반사)*. 이름의 D 가 추가될 때마다 *공격 형태가 진화*.
...
| **패킷 필터링 우회 (DoS 아님)** | Tiny Fragment | 첫 단편을 *작게* + 목적지 포트 숨김 |
| | Fragment Overlap | 단편 오프셋 *중첩*으로 포트 덮어쓰기 |

### Tiny Fragment / Fragment Overlap — 시험 함정

> *DoS 분류 표에 함께 등장*하지만 **자원 고갈 목적 ❌**. *IP fragment offset 취약점*을 이용해 *패킷 필터링 장비 우회*. **DoS 가 아닌 우회 공격**.
...
| DRDoS 의 IP 위조·증폭? | *반드시* 발생 |
| Tiny Fragment / Fragment Overlap 분류? | **DoS 가 아닌 우회 공격** |
...
## 📖 에피소드 3 — Smurf: Directed Broadcast + ICMP 증폭 + `no ip directed-broadcast`

### 장면. 한 번 ping 으로 1000 응답

> **Smurf Attack** = *출발지 IP 를 희생자 IP 로 위조* + *증폭 네트워크에 ICMP Echo Request 브로드캐스트* → *다수 ICMP Echo Reply 가 희생자에게 전달* → 서비스 거부.
...
### 우회 공격 2 — Tiny Fragment + Fragment Overlap (DoS 아님)

> 정확본의 *주의*: 이 두 공격은 *IP fragment offset 을 조작*하지만 **자원 고갈 목적 ❌, 패킷 필터링 우회 목적 ✅**.
...
| Tiny Fragment 분류? | **DoS 아닌 우회 공격** |
| Fragment Overlap 분류? | **DoS 아닌 우회 공격** |
...
> **DDoS** = *분산된 다수 좀비 PC·디바이스*에 의한 서비스 마비. **DoS 와 차이**는 *단일 컴퓨터 ❌, 분산된 다수 ✅*.
...
| **DoS / DDoS / DRDoS** | 1 / 분산 / 분산+반사 |
| **DRDoS 의 IP 위조·증폭** | *반드시* |
| **DoS 자원 소진** | Ping of Death · Land · Smurf · Teardrop · Bonk · Boink |
| **DoS 우회 (DoS 아님)** | **Tiny Fragment · Fragment Overlap** |
| **Ping of Death 탐지** | *분할된 ICMP* (본래 분할 ❌) |
| **이더넷 MTU** | **1500 byte** |
| **Land Attack** | *출발지 IP = 목적지 IP* |
| **Smurf 핵심** | *Directed Broadcast + ICMP Echo Reply 증폭* |
...
graph TD
  ...
  D3 --> D3a[공격자 다수<br/>반사서버 경유<br/>IP 위조 반드시<br/>증폭 반드시]
...
**읽는 법**: 노랑 → 빨강으로 *진화*. *D 가 늘어날수록 분산·반사*. DRDoS 만 *IP 위조·증폭 모두 반드시*.

---

## 🗺️ 마인드맵 2 — DoS 공격 7 종 분류 트리
...
graph TD
  ...
  R --> R3[Smurf<br/>Directed Broadcast<br/>+ ICMP 증폭]
  R --> R4[Teardrop / Bonk / Boink<br/>offset / 순서번호 조작]

  D --> B[패킷 필터링 우회<br/>DoS 가 아닌 우회 공격]
  B --> B1[Tiny Fragment<br/>첫 단편 작게<br/>목적지 포트 숨김]
  B --> B2[Fragment Overlap<br/>단편 중첩<br/>포트 덮어쓰기]

  R --> CMN1[공통 대응:<br/>OS 보안 패치<br/>패킷 필터링]

  B --> CMN2[공통 대응:<br/>재조합 후 검사<br/>방화벽 / IDS]
...
**읽는 법**: 노랑 = *DoS 가 아닌 우회 공격*. *시험 함정 1순위*.
...
| **Teardrop ↔ Bonk ↔ Boink** | offset 중첩 / 모두 1 / 중간 비정상 |
| **자원 소진 3 ↔ 우회 2** | Teardrop·Bonk·Boink ↔ Tiny Fragment·Fragment Overlap |
...
**A. DoS·DDoS·DRDoS 분류 + DoS 공격 (1~7)**
...
6. Tiny Fragment·Fragment Overlap 이 *DoS 가 아닌* 이유는?
...
| Bonk 조작? | 순서번호 *모두 1* |
| Boink 조작? | 정상 후 *중간 비정상* |
| Tiny Fragment·Fragment Overlap 분류? | **DoS 아닌 우회 공격** |
