...
> **연결 단원**: 본 단원은 [[cs/information-security/02.network-security/02.network-protocols-and-addressing|02 단원 §4 TCP]] 의 *3-way Handshake* 와 [[cs/information-security/02.network-security/01.network-fundamentals|01 단원 §1 OSI]] 의 *계층 구조*가 토대. *2계층 / 3계층 / 전송 계층 보안* 의 위치 차이가 핵심. *MITM 방어* (04 단원) 는 *SSL/TLS 인증서 검증*. *Shell Shock 의 명명 유래*인 *하트블리드*가 본 단원 §5 의 핵심.
...
| 회사 VPN 클라이언트 | **L2TP / IPsec** (3계층 터널) |
| `ipsec status` 의 SA 정보 | **Security Association** (양방향 = 2개) |
| TLS Handshake `ClientHello` / `ServerHello` | 완전 협상 13 단계의 1·2 |
...
> **이 단원의 본질**: *어느 계층에서 보안을 제공하는가* + *어떤 헤더·메시지를 주고받는가* + *어떤 다운그레이드·메모리·CBC 패딩 약점이 있는가* 의 3 축. **VPN 2계층 (PPTP·L2F·L2TP) ↔ 3계층 (IPsec)** + **AH 인증만 ↔ ESP 인증+기밀성** + **완전 협상 13 ↔ 단축 협상 2** + **5 취약점 다운그레이드 / 메모리 누출 / 패딩 오라클** 이 시험 1순위.
...
| # | 메시지 | 설명 |
|---|---|---|
| 1 | **Client Hello** | 지원 SSL/TLS 버전·암호도구·압축 + **client random (32bit 난수)** + 세션 ID (완전협상 = 빈 값) |
| 2 | **Server Hello** | 사용 버전·도구·압축 + **server random (32bit 난수)** + 세션 ID |
...
> *완전 협상*은 *6 (서버 측) + 6 (클라이언트 측) + 1 (초기)* 의 *13*. *단축 협상*은 *Hello 2개* 만으로 충분 (세션 정보 재사용).
...
| 완전 협상 단계 수? | **13** |
| 단축 협상 단계 수? | **2** |
| Client/Server Hello 의 난수 길이? | **32 bit** |
...
| **client/server random** | **각 32 bit 난수** |
...
## 🗺️ 마인드맵 4 — SSL/TLS 완전 협상 13 단계 sequenceDiagram
...
sequenceDiagram
    ...
    C->>S: 1. Client Hello<br/>버전·암호도구·압축 + client random
    S->>C: 2. Server Hello<br/>+ server random + 세션 ID
...
**읽는 법**: *서버 측 6 (1~6) → 클라이언트 측 5 (7~11) → 서버 측 2 (12~13) = 13 단계*. *8 단계의 사전 마스터 비밀 생성*이 키 파생의 출발점.
...
```
완전 협상 13:
  1. Client Hello              + client random (32 bit)
  2. Server Hello              + server random (32 bit)
...
22. 완전 협상 13 단계 vs 단축 협상 2 단계의 차이는?
...
| 완전 협상 단계? | **13** |
| 단축 협상 단계? | **2** |
| client/server random 길이? | **32 bit** |
| 사전 마스터 비밀? | 8단계 결과 |
...
[7주]   에피소드 9 (완전 협상 13 단계) — 마인드맵 4 (sequenceDiagram)
