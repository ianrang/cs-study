...
> **이 단원의 본질**: 2과목 출제 문제의 상당수는 *이 패킷이 어느 계층의 어떤 프로토콜이며 헤더의 어느 필드가 어떤 값일 때 무슨 의미인가*를 묻는다. **모든 비트열을 암기할 필요는 없고**, *길이·플래그·대표 필드·TCP/UDP/ICMP 숫자·시퀀스 다이어그램* 같은 **시험 1순위만 정량으로** 맞추면 된다.

### RFC 가 무엇인지 · 번호를 어디까지 외우나

> **RFC(Request for Comments)** 는 **IETF 등에서 인터넷 기술을 문서로 공표한 규격·설명서**이다. “표준 번호”처럼 외우되 **RFC 전부를 외울 필요는 없다**. 본 단원에서는 **단골만** 고른다 — 아래처럼 **티어**로 나누면 부담이 줄어든다.

| 암기 티어 | 예시 | 비고 |
|---|---|---|
| **티어 1** (자주 나옴) | IP **RFC 791**, ICMP **RFC 792**, UDP **RFC 768**, IPv6 **RFC 8200**, 사설 주소 **RFC 1918**, CIDR **RFC 4632** | 번호까지 맞추는 문제 출제 |
| **티어 2** (개념 위주) | TCP 통합 표준 **RFC 9293**, VLSM 참고 **RFC 1878** | “무슨 문서인가”보다 **역할·용어**가 우선인 경우 많음 |

> 정확본에 더 많은 RFC 가 나오면 **그때그때 목록만 추가**하고, 위 티어 1부터 압축해 두면 된다.
...
### 연결 고리 — IPv4 헤더와 TCP 헤더는 다른가

> **다르다.** 한 줄로 말하면 **IP 헤더 = 3 계층(IP 데이터그램의 앞부분)**, **TCP 헤더 = 4 계층(TCP 세그먼트의 앞부분)** 이고, 캡슐화하면 **`[이더넷] [IPv4 헤더 | TCP 헤더 | 애플리케이션 데이터]`** 순으로 붙는다. IP 의 **Protocol 필드**가 “안에 TCP(6)·UDP(17)·ICMP(1)” 인지 적어 두고, TCP 헤더에는 **포트·순서번호·플래그(SYN 등)** 가 있다 — **역할이 완전히 다르다.**

### 헤더 필드를 전부 순서대로 외워야 하나

> **시험 빈도 기준 우선순위**만 잡으면 된다. **IP**: 기본 길이 **20 byte**(최대 60)·**TTL·Protocol(6/17/1)·주소 32 bit·단편화 필드(bit 길이)** 가 상단. **TCP**: 기본 **20 byte**(최대 60)·**Seq/Ack 32 bit·Window 16 bit·6 플래그** 가 상단. 나머지 필드는 표를 보며 맞추면 된다.
...
#### 1480 과 1460 — 레이어가 다르다 (MSS 와 헷갈리지 않기)

> 표준 이더넷에서 **한 프레임에 실리는 IP 패킷** 크기 상한을 보통 **1500 byte** 로 본다(MTU). **IPv4 헤더가 최소 20 byte** 이면, IP 입장에서 그 안의 **데이터 영역(IP 페이로드)** 은 최대 **1500 − 20 = 1480 byte** 까지 들어갈 수 있다. 그 IP 페이로드 안에 **TCP 세그먼트**가 들어가는데, **TCP 헤더도 최소 약 20 byte** 를 쓰므로, TCP 입장에서 **애플리케이션 데이터만** 넣을 수 있는 상한(**MSS**)은 **1480 − 20 = 1460 byte** 가 된다.
>
> 정리: **1480 = IP 페이로드( TCP 헤더 + TCP 데이터 ) 상한**, **1460 = TCP 데이터(MSS, 헤더 제외) 상한** — 둘 다 맞고 “어느 계층에서 보는 크기인가”만 구분하면 된다. 에피소드 12 의 **MSS = MTU − IP(20) − TCP(20)** 과 같은 말이다.
...
|---|---|
| 이더넷 MTU? | **1500 byte** |
| IP 페이로드 최대 vs MSS ? | 헤더 각 20 가정 시 **1480**(IP 페이로드) vs **1460**(TCP 데이터=MSS) |
| 재조합 위치? | **최종 목적지에서만** |
| DF=1 의미? | *단편화 금지* |
| MF=1 의미? | *추가 단편 더 있음* |
| MF=0 의미? | *마지막 단편* |
...
### 연결 고리 — 왜 CIDR · VLSM 이 나오나

> **클래스(A/B/C 고정 마스크)** 만 쓰면 *작은 회선에 거대 블록이 붙거나*, 반대로 *주소가 빨리 바닥나는* 문제가 생긴다. 그래서 **클래스 구분을 버리거나 무시하고**, **원하는 길이만큼만 네트워크 접두(prefix)를 쓰자**는 게 **CIDR**. 한 회사 **내부**에서는 부서마다 필요한 호스트 수가 다르니 **서브넷 마스크 길이를 서브넷마다 다르게 쓰자**는 게 **VLSM**. 둘 다 **서브네팅(큰 블록을 잘게 자르기)** 과 세트로 이해하면 된다.
...
| 구분 | CIDR | VLSM |
|---|---|---|
| **한 줄** | **무계급 표기·라우팅** — `10.1.0.0/23` 처럼 **prefix 길이를 자유롭게** 쓴다 | **가변 길이 마스크** — **한 조직 안**에서 `/26` 과 `/28` 을 **동시에** 써 호스트 수에 맞게 자른다 |
| **비유** | 우편번호 체계 자체를 “고정 구역(A/B/C)” 에서 **유연한 구역번호** 로 바꿈 | 한 아파트 안에서 **층마다 호수 규칙을 다르게** 짜는 것 |
| **RFC** | **RFC 4632** | **RFC 1878** (클래스풀 서브넷 표 참고로 많이 인용 — 시험은 개념이 우선) |

> 둘 다 **주소 낭비를 줄이기 위한 기술**이지만, 시험에서는 **CIDR = 표기·인터도메인 쪽 개념**, **VLSM = 한 네트워크 아래 세분할** 로 구분하면 된다.
...
- "CIDR 한 줄?" → **무계급** + prefix 표기 + **RFC 4632**
- "VLSM 한 줄?" → **서브넷마다 마스크 길이 가변** + **RFC 1878**
...
| **강화된 보안성** | **IPsec 기본 제공** (확장 헤더로 협상·사용) |
...
### IPsec 과 VPN 은 같은 말인가

> **아니다.** **IPsec** 은 **네트워크 계층(3 계층)** 에서 동작하는 **인증(AH)·암호화(ESP)·키 교환(IKE 등)** 프로토콜 **묶음**이다. **VPN** 은 “망과 망·클라이언트와 망을 논리적으로 잇는 통로”라는 **용도·서비스 개념**이고, 그 터널을 만들 때 **IPsec** 을 쓰는 경우가 많을 뿐 **SSL VPN·TLS VPN** 등 다른 층의 기술도 있다. 문서 표의 “IPv6 에 IPsec 기본 제공”은 **IPv6 설계가 IPsec 과 잘 붙도록 확장 헤더 등이 마련되어 있다**는 뜻에 가깝다.
...
- "IPv6 자동 주소 설정?" → **SLAAC**
- "IPsec 과 VPN 동일?" → **아니오** — IPsec 은 **3 계층** 프로토콜 묶음 / VPN 은 **통로·서비스 개념**
...
> **`ping`** 은 운영체제에 따라 구현 세부는 다르지만, 개념적으로 **ICMP Echo 메시지를 보내고(Type 8)·받는(Type 0) 진단 도구**이다. 즉 **ping ≠ 별도 프로토콜** 이고 **ICMP 위에서 도는 사용자 프로그램**에 가깝다. **traceroute** 는 TTL 과 ICMP Time Exceeded(**Type 11**) 조합으로 동작한다 — 에피소드 4 **TTL** 과 한 줄로 이어진다.
...
- "ping 이 쓰는 프로토콜?" → **ICMP** (Echo Request **8** / Reply **0**)
...
#### 2MSL 과 MSL 이 정확히 무엇인가 · 몇 초인가

> **MSL(Maximum Segment Lifetime)** 은 TCP 세그먼트가 네트워크에 **존재할 수 있다고 가정하는 최대 수명**이다. **2MSL** 은 그 **두 배**를 TIME_WAIT 에서 기다리라는 뜻으로, (1) **지연되어 도착한 중복 세그먼트**가 소멸할 시간을 벌고, (2) **마지막 ACK 가 유실**됐을 때 상대의 **FIN 재전송**을 받을 수 있게 한다.
>
> 고전 문서에서는 MSL 을 **분 단위**로 두는 예가 많았지만, **실제 OS 마다 TIME_WAIT 타이머 값은 다르게 설정**된다. **정보처리기사류 시험**에서는 보통 **“TIME_WAIT = 2MSL 기다린다”는 관계와 목적**이 정답이지, **세계 공통 고정 초**를 묻지 않는 경우가 많다.
...
- "2MSL 이유?" → 마지막 ACK 분실 대비 · 지연 세그먼트 소멸 시간
- "2MSL 은 전 세계 동일 몇 초?" → **아님** — **MSL 의 2배** 라는 **관계**가 시험 핵심, 실제 초는 OS·설정 의존
...
> **MSS** = 한 번에 실을 수 있는 **TCP 페이로드(애플리케이션 데이터) 상한**. TCP·IP 헤더는 포함하지 않는다. **헤더가 최소 크기일 때** 흔히 쓰는 식은 다음과 같다.
...
> IP 페이로드 상한 **1480**(에피소드 4)과 비교하면 **1480 − TCP 헤더 ≈ MSS** 라고 기억하면 된다. 옵션 헤더가 길면 MSS 는 더 작아진다 — 시험 기본값은 **위 최소 헤더 가정**.
...
| 함정 | 정답 |
|---|---|
| MSS 계산식? | **MTU − IP(20) − TCP(20)** (최소 헤더 가정) |
| 이더넷 MSS? | **1460 byte** |
| **1480 vs 1460** | **1480** = IP 페이로드 상한 / **1460** = 그 안의 TCP **데이터** 상한(MSS) |
...
| Passive Close 상태? | ESTABLISHED → **CLOSE_WAIT** → **LAST_ACK** → CLOSED |
| TIME_WAIT 대기? | **2MSL** |
| 2MSL 이유? | 마지막 ACK 분실 대비 · 지연 세그먼트 소멸 |
| 2MSL 초 고정? | **아님** — **MSL 의 2배** 관계가 핵심 |
| 연결 거부 패킷? | **SYN → RST+ACK** |
| 연결 중단 패킷? | **RST+ACK** 또는 **RST** |
| MSS 계산? | **MTU − IP(20) − TCP(20)** (최소 헤더) |
| 이더넷 MSS? | **1460 byte** (= IP 페이로드 **1480** − TCP 헤더 **20**) |
| 빠른 재전송 트리거? | **동일 ACK 3회** |
| Well-known 범위? | **0 ~ 1023** |
| Registered 범위? | **1024 ~ 49151** |
| Dynamic 범위? | **49152 ~ 65535** |
