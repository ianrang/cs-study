...
| 매일 듣는 것 | 사실은 이 단원의 |
|---|---|
| `iptables -A INPUT -j DROP` | **iptables 체인·타겟** |
| `alert tcp any any -> ...` | **Snort 룰** (Martin Roesch) |
| 회사 *방화벽 + IDS + VPN 통합* 장비 | **UTM** (Unified Threat Management) |
| *SOC 의 SIEM 대시보드* | **빅데이터 기반 상관관계 분석** |
| *플레이북 자동화·SOAR* | SIRP + SOA + TIP |
| *EDR 알람* | 엔드포인트 행위 모니터링 |
| 사내 *Single Sign-On* | **SSO ⊂ EAM ⊂ IAM** |
| `/etc/init.d/nessusd restart` (8834/tcp) | **Nessus** (Tenable) |
| `chkrootkit -q` 의 INFECTED | 루트킷 점검 |
| *DMZ 영역* | 인터넷 ↔ 내부망 *중간 지점* |
| `CVE-2014-0160` | **CVE** 식별자 (MITRE) |
| *입력 검증·API 오용* | **CWE** 7 분류 |
| *KISA TI 보고서의 위협 인텔리전스* | **TIP** (SOAR 의 일부) |

> **이 단원의 본질**: *2과목 출제 분량 가장 큰 영역*. **(1) 인프라 보안 (VLAN·라우터·SecaaS) + (2) 도구 (Snort·iptables) + (3) 솔루션 13 종 (5 분류) + (4) 모니터링 (ESM→SIEM→SOAR) + (5) 단말 관리 (PMS·EMM·BYOD/CYOD) + (6) 방화벽·IDS/IPS·허니팟 + (7) 점검 도구 8 + (8) CVE/CWE** 의 *8 축*. **솔루션 분류·체인·상태·플래그·플래그번호·플러그인 포트 등 정량 식별자가 시험 1순위**.
...
> 통합 모니터링은 **ESM (1세대) → SIEM (2세대, 빅데이터) → SOAR (3세대, 자동화)** 의 *진화 흐름*.
...
> **SIEM** = ESM 의 *진화*. *보안장비 + 서버 + 네트워크 장비 + 어플리케이션* 광범위 + **빅데이터 기반 상관관계 분석** + 위협 *사전 차단*.
...
| **TIP** | **T**hreat **I**ntelligence **P**latform | *분석가 판단 보조* |
| **위협 인텔리전스 서비스** | — | *알려지지 않은 위협·APT 대응* |
...
> *ESM = 통합 (1세대) → SIEM = 빅데이터 + 상관관계 (2세대) → SOAR = 자동화 + 플레이북 (3세대)*. *분석 → 자동 대응*의 진화.
...
| SOAR 4 구성? | **SIRP / SOA / TIP / 위협 인텔리전스** |
...
|---|---|
| **인터넷 구간** | 외부 직접 접근 가능 |
| **DMZ 구간** | *인터넷·내부망 사이 중간*. 인터넷 접근 + 내부망 접근 허용 |
...
| 도구 | 영역 | 핵심 |
|---|---|---|
| **Nessus** | 시스템 취약점 | **Tenable 사** / **8834/tcp** / 클라이언트·서버 / 플러그인 |
| **Nikto** | 웹 취약점 | **CGI 스캔에 강함** |
| **Tripwire** | 무결성 | **`--init` (해시 생성) → `--check` (검증)** |
| **chkrootkit** | 루트킷 | **`-q` (quiet)** / 결과 **INFECTED** |
...
```bash
/etc/init.d/nessusd restart      # 데몬 기동 (8834/tcp)
...
| 함정 | 정답 |
|---|---|
| Nessus 개발사·포트? | **Tenable / 8834/tcp** |
| Nikto 의 강점? | **CGI 스캔** |
| Tripwire 명령? | **`--init` → `--check`** |
| chkrootkit 결과? | **INFECTED** |
...
| **ESM 3 구성** | 에이전트 / 매니저 / 콘솔 |
| **ESM → SIEM → SOAR** | 통합 → 빅데이터·상관관계 → 자동화·플레이북 |
...
| **네트워크 구간 3** | 인터넷 / DMZ / 내부망 |
| **Nessus** | **Tenable / 8834/tcp** |
| **Nikto** | **CGI 스캔** |
| **Tripwire** | `--init → --check` |
| **chkrootkit** | **`-q` / INFECTED** |
...
| **CWE 7** | 입력 검증 / 보안 기능 / 시간·상태 / 에러 처리 / 코드 오류 / 캡슐화 / API 오용 |
...
**읽는 법**: 노랑 = 각 분류 *대표 솔루션*. 녹색 = HSM (하드웨어 전용). *SSO ⊂ EAM ⊂ IAM* 의 포함 관계.
...
timeline
    ...
    2세대 SIEM : 전 범위 (서버·네트워크·앱)<br/>빅데이터 기반 상관관계<br/>로그 수집 8 단계<br/>실시간 모니터링
    3세대 SOAR : 자동화·플레이북<br/>SIRP (반복 업무 자동화)<br/>SOA (보안 시스템 통합)<br/>TIP (위협 인텔리전스)<br/>위협 인텔리전스 서비스
```

**읽는 법**: *통합 (1세대) → 빅데이터·상관관계 (2세대) → 자동화·플레이북 (3세대)* 의 *진화 흐름*. 시험에서 *세대 차이*와 *각 세대의 핵심 기능*을 묻는다.
...
graph TD
  ...
  T --> V[취약점 점검]
  V --> V1[Nessus<br/>Tenable<br/>8834/tcp<br/>시스템 취약점]
...
```
Nessus       — Tenable / 8834/tcp / 시스템 취약점
Nikto        — CGI 스캔에 강함 / 웹 취약점
Tripwire     — H-IDS / --init → --check
chkrootkit   — -q (quiet) / INFECTED
...
16. ESM 3 구성과 SIEM 핵심 분석은?
17. SOAR 4 구성 (SIRP·SOA·TIP·위협 인텔리전스) 차이는?
