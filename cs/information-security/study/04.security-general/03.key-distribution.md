...
| TLS 1.3 의 *서버 인증서 키가 털려도 과거 통신은 안전하다* (ECDHE 기준) | **Forward Secrecy** (PSK-only 는 예외) |
| 회사 도메인 로그인 (SSO·AD) 의 백엔드 | **Kerberos v5** 키 분배 |
| AWS CloudHSM, YubiHSM | **HSM** (Hardware Security Module) |
| AWS KMS, Vault transit secrets engine | **키 생명주기** 관리 (생성·저장·회전·암호 연산) — HSM/KMS 계열 |
| 카페 공공 Wi-Fi 에서 HTTPS 가 *왜* 안전한가 | **인증된 DH** + **MITM 방어** |
| Signal·WhatsApp 의 종단간 암호화 | **DHE + Double Ratchet + HKDF** |
| `ssh-keygen -t ed25519` 의 키 파일 | SSH **인증(서명)** 키 (**Ed25519**, RFC 8032) |
| OpenSSH 연결 시 `curve25519-sha256` KEX | **X25519** ECDH 키 합의 (RFC 7748) |
...
> **표기 주의**: 아래 7단계는 *운영 관점 학습용 요약*이다. NIST SP 800-57 Part 1 의 **공식 키 상태**는 Pre-activation, Active, Suspended, Deactivated, Compromised, Destroyed 등이다.
...
- "키 관리 7단계?" → **생성·배포·저장·사용·갱신·백업·폐기** (학습용 요약)
- "NIST 공식 키 상태 예?" → **Pre-activation, Active, Deactivated, Compromised, Destroyed** 등 (Part 1)
...
| 한계 | 설명 |
|---|---|
| **단일 장애점 (SPOF)** | KDC 침해 시 *신뢰 붕괴* — 장기 키 유출·티켓/세션 키 위조·인증 악용 가능 |
...
수학적 토대: **타원곡선 이산로그 문제 (ECDLP)** — 동등 보안 강도에서 **더 짧은 키** 사용 (NIST SP 800-57 Part 1).
...
- "TLS 1.3 의 기본 키 합의?" → **(EC)DHE** (Curve25519 또는 NIST P-256). **PSK-only** 는 예외
...
> **TLS 1.3** 은 정적 RSA·정적 DH **키 전송을 폐기**했고, **인증서 기반 핸드셰이크의 기본**은 **(EC)DHE** 이다. 이 모드에서는 Forward Secrecy 가 제공된다.
>
> **예외**: **PSK-only** (또는 (EC)DHE 없이 PSK 만 사용) 모드에서는 PSK 가 노출되면 과거·현재 트래픽이 위험해져 **FS 가 없다** (RFC 8446).
...
- TLS 1.3 cipher suite `TLS_AES_128_GCM_SHA256` = 기본 **(EC)DHE** (cipher suite 명에 키 교환 생략)

### 시험 함정

- "Forward Secrecy 제공 키 합의?" → **DHE·ECDHE** (임시 키). 정적 DH·RSA 키 전송 ❌
- "TLS 1.3 가 폐기한 키 교환?" → **RSA 키 전송** (정적 DH 키 교환 포함)
- "TLS 1.3 에서 FS 예외?" → **PSK-only** — PSK 노출 시 과거 세션도 위험
...
| TLS 1.3 사용 | ❌ **폐기** | ✅ **기본** ((EC)DHE; PSK-only 예외) |
...
- TLS 1.3 cipher suite 명에는 *키 교환 표시 자체가 없음* (기본 (EC)DHE 라 생략)
...
- AWS KMS 의 envelope encryption = CMK 로 **데이터 키** 생성·랩핑 (앱에서 HKDF 등 추가 도출 가능)
...
| **ECDH** | 공개키·합의 | SP 800-56A Rev. 3 | 타원곡선 이산로그 (ECDLP) | 정적 ❌ / **ECDHE ✅** | **기본** (PSK-only 예외) |
| **STS** | 공개키·합의 (인증) | 1992 | 인증된 DH (서명) | DHE 사용 시 ✅ | 인증된 DH **선행 개념** |
...
3. **TLS 1.3 의 결단** (RFC 8446, 2018): RSA 키 전송 폐기 → 인증서 핸드셰이크 기본 **(EC)DHE** (PSK-only 는 FS 예외)
...
timeline
    ...
    2018  : RFC 8446 — TLS 1.3 (RSA 키 전송 폐기, (EC)DHE 기본)
...
**읽는 법**: 키 분배 역사는 *공격에 대한 응답*. Otway-Rees 가 1987 에 나온 이유 = 1981 Denning-Sacco 결함. STS 가 1992 에 나온 이유 = DH 의 MITM 약점. TLS 1.3 가 2018 에 RSA 키 전송을 폐기한 이유 = **(EC)DHE 기본화** 로 FS 확보 (PSK-only 예외).
...
graph TD
  ...
  ECDH --> ECDHE[ECDHE 임시 키<br/>✅ Forward Secrecy<br/>TLS 1.3 기본]
```
...
| TLS 1.3 폐기 키 교환? | **RSA 키 전송** (정적 DH 포함) |
| TLS 1.3 FS 예외? | **PSK-only** — PSK 노출 시 과거 세션 위험 |
| Ed25519 vs X25519? | **Ed25519** = 서명(RFC 8032) / **X25519** = ECDH(RFC 7748) |
