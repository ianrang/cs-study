...
> **이 단원의 본질**: 디지털서명은 *MAC·해시와 달리* **제3자 검증 기반 부인 방지(non-repudiation)** 를 제공하는 메커니즘이다. (법적 부인 방지는 PKI·신원 확인·키 관리·시점 증명 등과 함께 본다.) 그러나 디지털서명 단독으로는 *"이 공개키가 정말 그 사람의 것인가"* 를 증명할 수 없다. 그 답이 **PKI** 다 — 신뢰할 수 있는 CA 가 *공개키와 신원을 묶어주는* 인프라.
...
→ 해시함수가 두 문제를 *동시에* 해결 (임의 길이 → 고정 길이, 일방향성 + 충돌 저항성). 그래서 **RSA·ECDSA·DSA 등 대부분의 디지털서명은 *해시 후 서명* 구조** (`Sign(sk, H(M))`).

> **예외 — EdDSA** (FIPS 186-5, RFC 8032): 별도 message digest 를 먼저 해시해 넣는 hash-then-sign 구조가 *아니다*. EdDSA 스킴 내부에서 메시지를 처리한다.

### 개발자 비유

- JWT 의 서명 부분 = `Sign(privateKey, base64(header) + "." + base64(payload))` — RS256·ES256 은 헤더+페이로드를 *해시*한 후 서명
- Git 태그 서명 (`git tag -s v1.0`) = SHA-1 해시 후 GPG 키로 서명

### 시험 함정

- "서명 시 메시지 자체를 서명?" → **대부분 아니오**. *해시*를 서명 (RSA 입력 길이 제한 + 효율성). **EdDSA 는 예외**
...
- "왜 디지털서명만 부인 방지?" → 개인키는 *송신자만* 가짐 → **제3자**가 MAC 과 달리 출처 판정 가능 (법적 효력은 PKI 등 추가 요소)
...
- **확률적 서명** — 선택 메시지 공격(EUF-CMA) 하 위조 불가능성 *증명됨*
...
|---|---|---|
| 결정성 | 결정적 | **확률적** |
| 안전성 증명 | 약함 | **EUF-CMA** (선택 메시지 위조 불가) 증명 |
...
- "PKCS#1 v1.5 vs PSS?" → **PSS 가 확률적 + EUF-CMA 증명**. FIPS 186-5 새 응용 권장
...
> **RFC 6979 (Pornin, 2013)**: "*결정론적 k* 를 *해시*에서 도출하자. 임시 k 를 무작위로 만들 필요 없이, 메시지 + 개인키의 HMAC 으로 도출하면 *불량 RNG 로 인한 k 재사용·예측* 을 실질적으로 막는다."

```
k = HMAC-SHA-256(개인키, 메시지 해시) 를 변형해 도출
```

→ Sony PS3 같은 **k 재사용 사고**를 크게 완화 (구현 버그·부채널까지 모두 제거하진 않음). Bitcoin·OpenSSL·libsodium 모두 RFC 6979 채택.
...
| 개선 | 의미 |
|---|---|
| **결정론적** | nonce 를 스킴에 내장 → **불량 RNG 로 인한 k 재사용** 위험 설계상 완화 (DSA·ECDSA 와 구조적 차이) |
| **부채널 완화** | 일정 시간 연산을 목표로 *곡선·구현* 설계 (절대적 저항 아님) |
...
### FIPS 186-5 승인 3종 + DSA 레거시

> **FIPS 186-5 (2023) 서명 생성 승인**: **RSA**, **ECDSA**, **EdDSA** 3가지. **DSA** 는 신규 사용 금지, *기존 서명 검증* 용도만 유지.

| 알고리즘 | 보안 토대 | 키 길이 (128 bit 보안) | FIPS 186-5 |
|---|---|---|---|
| **RSA-PSS / PKCS#1 v1.5** | 인수분해 | 3072 bit | **승인** (PSS 신규 권장) |
| **ECDSA** | 타원곡선 이산로그 | 256 bit | **승인** |
| **EdDSA** (Ed25519·Ed448, RFC 8032) | 타원곡선 이산로그 | 256/224 bit | **승인 (2023~)** |
| **DSA** | 이산로그 | 3072 bit | **신규 금지** (검증만) |

### 개발자 비유

- `ssh-keygen -t ed25519` = Ed25519 키쌍 (2024년 SSH 기본 권장)
- JWT `EdDSA` 알고리즘 = Ed25519 서명
- WireGuard VPN = **Curve25519(X25519)** 키 교환 + **ChaCha20-Poly1305** + **BLAKE2s** (Noise IK, Ed25519 서명 아님)

### 시험 함정

- "EdDSA 의 두 변형?" → **Ed25519 (128 bit)** · **Ed448 (224 bit)** (RFC 8032)
- "EdDSA 가 DSA·ECDSA 대비 강점?" → **결정론적 nonce** (불량 RNG·k 재사용 완화) + 부채널 **완화** 설계
- "FIPS 186-5 서명 승인 알고리즘?" → **RSA·ECDSA·EdDSA** 3종. **DSA 신규 금지**
...
### TBSCertificate 주요 필드 (RFC 5280 §4.1)
...
| 확장 | Extensions | v3 **선택** 필드 — 실무 핵심은 아래 § |
| CA 서명 | Signature | 위 필드 전체에 대한 *CA 의 디지털서명* |

### v3 의 핵심 — *확장(extensions)* (시험: SAN·KU·EKU·BC)
...
| 변화 | 의미 |
|---|---|
| **공인인증서 우월적 효력 폐지** | 공인·사설 **형식상 차별 폐지** — 전자적 형태만으로 효력 부인 불가, 당사자 약정·법령에 따름 (입증력·신뢰는 사업자·용도별 상이) |
...
2. **임시 키 k 의 위험**: DSA·ECDSA 의 약점. RFC 6979·EdDSA 가 *결정론적 nonce* 로 RNG·k 재사용 위험 완화
3. **FIPS 186-5 의 의미**: **승인 3종** (RSA·ECDSA·EdDSA), DSA 신규 금지 (2023)
...
graph TD
  ...
  RSA --> PSS[RSA-PSS<br/>RFC 8017<br/>확률적·EUF-CMA]

  EC --> DSA[DSA<br/>NIST 1991<br/>❌ FIPS 186-5 신규 금지]
  EC --> ECDSA[ECDSA<br/>FIPS 186-5<br/>NIST P-256 / secp256k1]
  EC --> EDDSA[EdDSA<br/>RFC 8032 / FIPS 186-5 2023<br/>결정론적·부채널 완화]
...
| PKCS#1 v1.5 ↔ RSA-PSS | 결정적 ↔ **확률적** + EUF-CMA 증명 |
...
14. EdDSA 가 DSA·ECDSA 대비 강점(결정론적 nonce·부채널)은?
...
|---|---|
| 디지털서명 5 요건? | 위조 불가·인증·부인 방지·변경 불가·재사용 불가 |
| 메시지 자체를 서명? | **대부분** ❌ — **해시** 후 서명. **EdDSA 예외** (FIPS 186-5) |
| MAC 의 부인 방지? | ❌ 대칭키 공유로 검증자도 만들 수 있음 |
| FIPS 186-5 서명 승인? | **RSA·ECDSA·EdDSA** 3종. **DSA 신규 금지** (검증만) |
| PKCS#1 v1.5 vs PSS? | 결정적 ↔ **확률적** + **EUF-CMA** 증명 |
| DSA·ECDSA 약점? | **임시 키 k 재사용** 시 개인키 즉시 유출 |
| RFC 6979 해결책? | **결정론적 k** (HMAC 기반) |
| EdDSA 두 변형? | **Ed25519** (128) · **Ed448** (224) |
| EdDSA 의 강점? | **결정론적 nonce** (k/RNG 완화) + 부채널 **완화** |
| WireGuard 암호 구성? | X25519 + ChaCha20-Poly1305 + BLAKE2s (**Ed25519 아님**) |
...
[2주]   에피소드 4·5·6·7 (서명 알고리즘 4종) — 마인드맵 1·2 시간선·분류
...
[10주]  04·05·06·03 통합 — 인수분해(05) ↔ RSA서명(04) / 이산로그(05) ↔ ECDSA·EdDSA(04) / 해시(06) ↔ 대부분 디지털서명(04) / 키 분배(03) ↔ PKI(04)
```
