...
> **시험 함정**: 위험·적응형 신호는 **AAL(인증 보증 수준)을 대체하지 않는다**. 본 인증 강도와 별개의 *보조 신호*로 이해한다 (NIST Digital Identity Guidelines 취지).

### NIST SP 800-63 AAL 등급 (Authenticator Assurance Level)

> **AAL**: 인증 보증 수준 3단계. 판본마다 세부 문구가 다를 수 있어 *핵심 키워드* 위주로 외운다. 최신 **SP 800-63-4** 본문은 AAL3을 **공개키 암호 프로토콜**로 키 소유 증명, **피싱 저항(phishing-resistant)** 인증기, **비수출(non-exportable)** 인증 키, **서로 다른 2요소** 증명 등으로 규정한다.

| AAL | 요구사항 (요지) | 인증기 예시 |
|---|---|---|
| **AAL1** | 단요소 허용 (지식 또는 소유 또는 존재 1개) | 패스워드 단독 |
| **AAL2** | *다요소 필수* (서로 다른 2개 요소) | 패스워드 + OTP, 패스워드 + 푸시 인증 |
| **AAL3** | 공개키 프로토콜 + **피싱 저항** + **비수출 키** + **2요소** (재전송·의도 증명 등 세부 요건 포함) | 보안키+PIN/생체, 스마트카드·TPM 등 (*배포·검증 수준에 따라 등급 달라짐*) |

### 시험 함정

- "5요소 인증?" → NIST 정식은 **3요소**. 위치·행동은 부가 요소
- "AAL3 의 핵심 (최신본 기준)?" → **공개키 기반 인증** + **피싱 저항** + **비수출 인증 키** + **2요소**. (*구판·교재에선 **하드웨어 보호**·**검증자 임퍼스네이션(피싱) 저항** 표현이 병기되기도 함*)
...
## 📖 에피소드 3 — 패스워드: 통념의 붕괴 (2017)와 최신 길이 기준 (800-63-4)
...
### 장면 3. 최신 NIST — SP 800-63-4 (암호 길이 재정의)

> **[NIST SP 800-63-4](https://pages.nist.gov/800-63-4/sp800-63b.html)** 계열(공개 Digital Identity Guidelines)의 **password(암호·패스프레이즈)** 요건 요약:
> - **단일 요소** 인증에만 쓰는 암호: 검증자·CSP는 **최소 15자** (SHALL).
> - **다요소 인증** 과정에만 쓰이는 암호: **최소 8자** (SHALL, 더 짧게는 불가).
> - **최대 길이**: **최소 64자까지 입력 허용** 권장(SHOULD) — 긴 패스프레이즈·Unicode 지원과 연계.

> **시험 함정**: 교재·기출이 **2017년 SP 800-63B**만 전제하면 **단일 요소 최소 8자·검증자 64자 허용**이 정답인 경우가 있다. 문제가 **“최신 NIST”** 를 명시하면 **15 / 8 / 64(권장)** 를 구분한다.
...
- "패스워드 길이 (2017 · 800-63B 당시 기출)?" → 단요소 **최소 8자**, *검증자는 최대 길이 **최소 64자** 허용 권장*에 맞춰 구현
- "패스워드 길이 (최신 SP 800-63-4)?" → 단요소 **15자 이상** / MFA 전용 암호 **8자 이상** / 최대 **64자 이상 허용** 권장
...
- **시각 동기 필수**, 시계 오차를 무시할 수 없어 검증 시 **인접 time step**을 허용하는 구현이 일반적 — RFC·관행상 **대개 현재 step ±1** (X=30초면 약 ±30초) 을 쓰지만, **허용 폭은 표준이 고정한 값이 아니라 서버·제품 정책**이다
...
|---|---|---|
| 카운터 원천 | **이벤트** (인증 시도) | **시간** |
| 동기화 요구 | 카운터 동기 윈도우 | **시각 동기** (통상 ±1 step 윈도우·**구현 의존**) |
...
- "TOTP 시각 동기 윈도우?" → 관행상 **대개 ±1 time step** (30초면 ±30초) — **실제 허용 범위는 구현 정책**
...
> RFC 2289 가 권장 해시로 MD5·SHA-1 명시 (1998 시점). 목적은 **재사용·재전송에 취약한 패스워드 체계**를 보완하는 일회용 패스워드. **RFC 2289**는 *(오늘날 기준)* **레거시 방식**으로, **현대 시스템 신규 도입은 비추** — 시험·역사적 맥락 위주로 암기한다.
...
> **FIDO2 / WebAuthn** 은 **AAL3을 만족할 수 있는 대표 기술**이다(공개키·피싱 저항·비수출 키·2요소 등을 **구현으로** 충족할 때).  
> 다만 **모든 FIDO2 배포가 자동으로 AAL3은 아니다** — 인증기 종류, PIN/생체 여부, 키 **비수출성**, RP·검증자 설정에 따라 **AAL2 이하**에 머무를 수 있다. **단요소 FIDO2** 도 가능하지만 *권장되지 않는다*.
...
- "FIDO2 의 AAL?" → **구현·설정에 따라 다름**. *AAL3을 목표로 할 수 있는 대표 스택*이지 **FIDO2 = 항상 AAL3 단정**은 금지
...
| 동기화 | 카운터 윈도우 | 통상 **±1 time step** (관행·**구현 의존**) | *불필요* |
...
**읽는 법**: 인증의 진화는 *공격에 대한 응답*. OIDC (2014) 가 나온 이유 = OAuth 2.0 (2012) 의 인증 오용. NIST 2017 패스워드 정책 개정 = *사용자 행동 연구*에 따른 통념 폐지(이후 **SP 800-63-4**에서 암호 길이가 다시 세분화됨). FIDO2 (2019·2021) = 패스워드 약점·피싱 완화를 겨냥한 대표 스택(**AAL3 달성 가능**, 배포마다 자동 AAL3은 아님).
...
graph TD
  ...
  S -.보급 환경.-> S2[레거시 RFC 2289<br/>신규 도입 비추·시험 맥락]
...
graph TD
  ...
  Q --> Q3{패스워드리스?}
  Q3 -- 예 --> FIDO[FIDO2<br/>WebAuthn + CTAP2<br/>AAL3 달성 가능]

  style FIDO fill:#e1f5e1
  style OAU fill:#fff3e0
```

**읽는 법**: OAuth 2.0 은 *인가만*. 인증 필요하면 OIDC (OAuth 위 ID 토큰). 피싱·서버 침해 위협까지 낮추려면 FIDO2 등 **피싱 저항** 인증기(구현에 따라 AAL 상이).
...
| AAL1 ↔ AAL2 ↔ AAL3 | 단요소 / 다요소 / 공개키·피싱 저항·비수출 키·2요소 (*구판·기출: 하드웨어+검증자 임퍼스(피싱) 저항*) |
...
4. AAL1·2·3 의 핵심 차이는?
5. AAL3 의 핵심 요건(SP 800-63-4 요지: 공개키·피싱 저항·비수출 키·2요소)은?
6. MFA 가 보안을 높이는 *진짜* 메커니즘?

**B. 패스워드·PIN·시도-응답 (7~10)**
7. NIST 2017 의 패스워드 정책 변화 4가지? (보충: **SP 800-63-4** 단요소 암호 **15자+**, MFA 전용 **8자+**, 최대 길이 **64자+ 허용** 권장)
...
| AAL3 핵심 요건? | **공개키 프로토콜** + **피싱 저항** + **비수출 키** + **2요소** (*구판·기출: 하드웨어+임퍼스(피싱) 저항*) |
| NIST 2017 패스워드? | 주기 변경 폐지 + 복잡도 폐지 + 차단 목록 |
| 패스워드 길이 (2017·구 800-63B)? | 단요소 **최소 8자**, 검증자 **최대 64자 이상 허용** 권장 |
| 패스워드 길이 (**SP 800-63-4**)? | 단요소 **15자+** / MFA 전용 **8자+** / 최대 **64자+ 허용** 권장 |
...
| TOTP 시각 윈도우? | 관행상 **대개 ±1 step** — **실제는 서버·제품 정책** |
...
| FIDO2 AAL? | **구현·설정에 따라 다름** — AAL3 **달성 가능**한 대표 스택 (항상 AAL3 ❌) |
| MAC 부인 방지? | ❌ (대칭키 공유) |
| HMAC ipad·opad? | **ipad=0x36, opad=0x5C**, 블록 길이만큼 반복 |
...
[6주]   에피소드 12 (FIDO2) — AAL3 달성 조건과의 관계(항상 AAL3 아님)
