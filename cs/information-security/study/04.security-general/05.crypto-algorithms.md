...
| JWT 의 마지막 점 뒤 부분 (`signature`) | **MAC**(HS256·HMAC) 또는 **디지털서명**(RS256·ES256·EdDSA) |
| `bcrypt.hash(password)` | **비밀번호 KDF** (06 단원 해시·MAC과 구분) |
...
(*블록·스트림 분류는 대칭키에만 적용. 비대칭키는 별도 분류 — RSA 등은 모듈러스 크기에 따라 한 번에 처리 가능한 메시지 길이에 제한.*)
...
→ DES·3DES **신규 시스템 사용 비권장**. NIST SP 800-67 Rev. 2 는 **2024년 1월 1일 철회(withdraw)**.
...
- Rijndael ↔ AES? → **AES = Rijndael 중 FIPS 197 범위만 표준화** (블록 128 bit 고정, 키 128/192/256, 라운드 10/12/14). Rijndael 은 후보명·더 넓은 매개변수 조합.
...
→ NIST **SP 800-38 시리즈**에 걸쳐 운영 모드가 정의된다. **SP 800-38A** 는 기밀성 모드 5종(ECB·CBC·CFB·OFB·CTR), GCM·CCM·XTS·KW 는 각각 **38D·38C·38E·38F**.

### 운영 모드 종합 표 (38 시리즈 9종)

| 모드 | 표준 | 병렬화 | IV/Nonce | 인증 | 용도 |
|---|---|---|---|---|---|
| **ECB** | 38A | ✅ | ❌ | ❌ | **사용 부적절** (같은 평문→같은 암호문, 펭귄 사고) |
...
→ NIST SP 800-38A 도 일반 암호화에 *ECB 비권장*. 같은 평문 블록이 같은 암호문 블록이 되어 **패턴이 노출**된다.
...
→ **TLS 1.3** (RFC 8446)은 **AEAD cipher suite** 만 사용 — 대표 **AES-GCM · AES-CCM · ChaCha20-Poly1305** 계열. AEAD = 기밀성 + 무결성 + 인증을 *한 번에*.
...
> **RSA-OAEP** 는 공개키 **암호화**에서 **IND-CCA2** 목표, **AES-GCM** 은 **AEAD**(기밀성+무결성)가 현대 표준. (디지털서명의 EUF-CMA 는 04단원)
...
```
1. 큰 소수 p, q 선택  (128 bit 보안 목표면 n≈**3072** bit — SP 800-57, 아래 표 참조)
2. n = p · q                  (모듈러스, 공개)
...
| IND-CCA2 (암호화) | ❌ | ✅ (OAEP, RFC 8017) |
| 표준 | 사용 금지 | **RFC 8017 (PKCS#1 v2.2)** |

### 양자 위협

**Shor 알고리즘**이 RSA 를 *다항 시간*에 깬다. 양자 컴퓨터가 충분히 커지면 RSA 사망. → **NIST PQC** — **FIPS 203**(ML-KEM, 구 Kyber)·**FIPS 204**(ML-DSA, 구 Dilithium)·**FIPS 205**(SLH-DSA, SPHINCS+) **2024년 8월 발표**.
...
**1985년, Neal Koblitz 와 Victor Miller** *동시에* 제안: "타원곡선 위 이산로그(ECDLP) — *동등 보안 수준*에서 RSA·DH 보다 *훨씬 짧은 키*로 구현 가능."
...
| **격자 기반** (ML-KEM·ML-DSA) | LWE·SIS 문제 | **FIPS 203·204·205** (2024) | **✅** |
...
timeline
    ...
    2023  : SP 800-67 Rev.2 철회 예고 / FIPS 186-5 (EdDSA 포함)
    2024  : SP 800-67 철회 · FIPS 203/204/205 (PQC)
```
...
graph TD
  ...
  Q --> Q4{ECB?}
  Q4 -.패턴 노출.-> NO[❌ 같은 평문=같은 암호문]
```

**읽는 법**: 위에서부터 *질문에 답해 내려가면* 정답 모드가 나온다. ECB 는 어떤 경로로도 도달하지 않음 — *NIST 비권장(패턴 노출)*.
...
**D. 운영 모드 (21~24)**
21. ECB 가 사용 부적절한 이유? (패턴 노출)
22. CBC 의 두 가지 약점?
23. CTR 의 가장 큰 약점?
24. TLS 1.3 AEAD cipher suite 대표 3계열?
...
| HIGHT 의 블록? | 64 bit (4총사 중 유일) |
| LEA 의 구조? | ARX |
| ECB 약점? | 같은 평문 = 같은 암호문 → 패턴 노출 (NIST 비권장) |
| CBC 약점? | 패딩 오라클 + 병렬 ❌ |
| CTR 약점? | 인증 없음, Nonce 재사용 ❌ |
| SP 800-38A 모드? | ECB·CBC·CFB·OFB·CTR (5종) |
| AEAD 3계열? | AES-GCM · AES-CCM · ChaCha20-Poly1305 |
| TLS 1.3 cipher? | AEAD만 — 위 3계열 대표 |
| Rijndael ↔ AES? | AES = Rijndael 중 블록128·키128/192/256만 (FIPS 197) |
| bcrypt? | 비밀번호 KDF (일반 해시 아님) |
| JWT signature? | HS256=MAC / RS·ES·Ed=디지털서명 |
| 양자 후 RSA·ECC? | ❌ Shor |
...
| RSA-OAEP 우월? | 확률적 패딩 + IND-CCA2 (RFC 8017) |
| PQC 표준? | FIPS 203 ML-KEM · 204 ML-DSA · 205 SLH-DSA (2024) |
| 3DES? | 신규 사용 비권장, SP 800-67 Rev.2 철회(2024) |
