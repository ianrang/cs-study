...
**기능 요건**(해시로 동작) + **보안 요건**(NIST: collision / preimage / second preimage) 으로 나눈다.

| 구분 | # | 이름 | 영문 | 의미 |
|---|---|---|---|---|
| 기능 | 1 | **압축성** | Compression | 임의 길이 입력 → 고정 길이 출력 |
| 기능 | 2 | **계산 효율성** | Efficiency | 입력에서 해시값을 빠르게 계산 |
| 보안 | 3 | **역상 저항성** | Preimage Resistance | (일방향성) h 가 주어졌을 때 H(x)=h 인 x 찾기 어려움 |
| 보안 | 4 | **제2역상 저항성** | Second Preimage Resistance | (약한 충돌) **주어진** x 에 대해 H(x')=H(x) 인 *다른* x' 찾기 어려움 |
| 보안 | 5 | **충돌 저항성** | Collision Resistance | (강한 충돌) H(x)=H(x') 인 *임의의* 두 x≠x' 쌍 찾기 어려움 |

### 위계 — 시험용 직관 (엄밀 수학 함의 아님)

```
충돌(강한) 저항성 ≥ 제2역상(약한) 저항성 ≥ 역상(일방향) 저항성
```

- **충돌**이 깨지면 → **제2역상**·**역상**도 깨지는 방향 (강한 → 약한)
- **제2역상**만 있다고 **충돌**이 자동으로 따라오지는 **않음** (역방향 함의 ❌)
- NIST 도 보안 핵심을 **collision / preimage / second preimage** 3종으로 구분

### 시험에서 구분하는 정의

| 저항성 | 시험 정의 |
|---|---|
| **역상(일방향)** | 해시값 h → 원문 x 찾기 어려움 |
| **제2역상(약한 충돌)** | **주어진** x → 같은 해시의 **다른** x' 찾기 어려움 |
| **충돌(강한)** | **임의의** x≠x' 쌍에서 같은 해시 찾기 어려움 |

### 보안 강도 공식 (생일 공격)

> **n bit 해시의 보안** (시험·평균 복잡도):
> - 역상·제2역상 저항성 → **2^n** 시도 (NIST SP 800-57 은 메시지 최대 길이 등을 반영할 수 있음)
> - **충돌 저항성 → 2^(n/2)** ← 생일 공격
>
> 예: SHA-256 → 역상·제2역상 **256 bit**, **충돌 저항 128 bit**.

### 시험 함정

- "SHA-256 의 충돌 저항 보안?" → **128 bit** (n/2). 256 ❌
- "약한 충돌 저항이 있으면 강한 충돌 저항도 있다?" → ❌. 위계는 *강한 → 약한* 방향만
- "5요건?" → 기능 2(압축·효율) + 보안 3(역상·제2역상·충돌)
...
→ 1990년대 SSL/TLS·디지털서명·체크섬 표준이 됨. 시험은 **출력 128 bit**·**M-D**·**2004 Wang 충돌**이 핵심.
...
### 장면 4. 2030년 — NIST 단계적 퇴출

NIST SP 800-131A: **연방·민감 시스템**에서 SHA-1 **단계적 퇴출**(2030-12-31 목표), **SHA-2/SHA-3** 전환. 전 세계 모든 용도 일괄 금지는 아님.
...
- "SHA-1 의 상태?" → 2017 SHAttered 충돌. NIST 연방 표준에서 단계적 퇴출(2030 목표).
...
**LSH (Lightweight Secure Hash)** — 2014, KISA 발표. TTAS.KO-12.0276 + KS X 3262. 구조: **Wide-pipe Merkle-Damgård**.
...
| **SHA-1** | 160 | FIPS 180-1 (1995) | Merkle-Damgård | ❌ 충돌 발견. NIST 연방·신규 비권장 (2030 퇴출 목표) |
...
**대응 3가지**:
- **HMAC** (RFC 2104, FIPS 198-1 → SP 800-224 이전 예정) — 이중 해시로 우회
...
세 암호학자가 발표 — "단순 `H(K||M)` 은 길이 연장에 취약. *이중 해시*로 우회하자." → **RFC 2104**, **FIPS 198-1** (시험). NIST 는 **SP 800-224** 로 이전 예정(2025 공지).
...
| MAC | 기반 | 표준 | 주요 용도 |
|---|---|---|---|
| **HMAC** | 해시함수 | RFC 2104, FIPS 198-1 (→ SP 800-224) | 일반, TLS, IPsec, JWT |
...
2010  PBKDF2 (NIST SP 800-132, RFC 8018)     — HMAC 반복 (레거시·FIPS-140)
...
| GPU 저항 | ❌ | 부분적 | ✅ | ✅ |
| ASIC 저항 | ❌ | 부분적 | ✅ | ✅ |
| 권장 | 레거시·**FIPS-140** | 호환·간단 | GPU 저항 필요 | **신규 기본 (OWASP)** |
...
### OWASP Cheat Sheet — 최소 vs 운영(보수적)

| 알고리즘 | OWASP **최소** | 문서 **운영 권장**(보수적) |
|---|---|---|
| **Argon2id** | 19 MiB, t=2, p=1 | m=64 MB, t=3, p=4 (또는 37 MB, t=4) |
| **bcrypt** | work factor **≥ 10** | cost **≥ 12** (4,096 회) |
| **scrypt** | N=2^17, r=8, p=1 | 동일 |
| **PBKDF2-SHA-256** | **600,000+** (FIPS-140 필요 시) | 동일 |

> 신규는 **Argon2id** 우선. **PBKDF2** 는 레거시·FIPS-140 준수 환경에서도 허용(600k+).
...
- "PBKDF2 가 Argon2 보다 약한 이유?" → *메모리-하드 아님* → GPU·ASIC 가속. 단, **FIPS-140** 등에서는 PBKDF2 600k+ 허용
...
timeline
    ...
    2030  : SHA-1 NIST 연방 퇴출 목표 (SP 800-131A)
```
...
graph TD
  ...
  H2MD --> SHA1[SHA-1 ❌ 충돌·NIST 퇴출<br/>SHAttered 2017]
...
graph TD
  ...
  MH --> HMAC[HMAC<br/>RFC 2104, FIPS 198-1<br/>→ SP 800-224]
...
graph TD
  ...
  Q2 -- 레거시·FIPS-140 --> PBK[PBKDF2-SHA-256<br/>600,000+ 회]
...
FIPS 198-1 — HMAC (2008, SP 800-224 이전 예정)
...
SHA-1    ❌ 2017 충돌 (SHAttered) — NIST 연방·신규 비권장
...
| 함정 | 정답 |
|---|---|
| 해시 5요건? | 기능 2(압축·효율) + 보안 3(역상·제2역상·충돌) |
| 역상 vs 일방향? | **역상(Preimage) 저항성** = 일방향성 (선상 ❌) |
| 충돌 위계? | 강한→약한 방향만. 약한→강한 ❌ |
| 충돌 저항 보안 강도? | n bit → **2^(n/2)** (생일 공격) |
...
| 패스워드 해싱 권장? | **Argon2id** 우선. PBKDF2 600k+ = FIPS-140 |
| OWASP Argon2id 최소? | 19 MiB, t=2, p=1 |
| MD5 라운드? | **4 라운드**(64 스텝). 출력 **128 bit** |
| SHA-1 2030? | NIST **연방** 단계적 퇴출, 전세계 일괄 금지 아님 |
| HMAC 표준? | 시험: RFC 2104, FIPS 198-1 |
...
| LSH 의 출력? | 224~512 bit. Wide-pipe M-D. KCMVP |
