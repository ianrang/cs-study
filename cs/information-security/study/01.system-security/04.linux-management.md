...
> `**pwconv` = password convert**. shadow *활성화* (passwd → shadow) / `pwunconv` = *비활성화* (shadow → passwd 통합). 03 §2-4 참조.
...
| **R 계열**       | 인증 없이 관리자 원격       | `rlogin(원격 로그인)` · `rsh(원격쉘)` · `rexec(원격에서 프로그램 실행)`        |
...
### 보충 — syslog·rsyslog 와 TCP/UDP (로그인 아님)

| 구분 | 설명 |
|------|------|
| **역할** | `syslogd`·`rsyslogd`는 **사용자 로그인(인증) 서비스가 아니다.** 커널·데몬·응용이 남기는 **로그 메시지를 수집·분배**하는 데몬이다. |
| **로컬** | 보통 **파일 기록**과 **유닉스 도메인 소켓**(`/dev/log` 등)으로 로컬에서 메시지를 받는다. |
| **원격** | “원격 로그 서버”는 이 메시지를 **네트워크(주로 UDP/TCP 514)** 로 **다른 호스트에 전달·보관**하기 위한 설정이다. 즉 **로그를 보내기 위해** TCP 또는 UDP를 쓰는 것이지, **원격 서버와의 모든 통신이 UDP뿐**이라는 뜻은 아니다. |
| **시험 관점** | 점검 문항·교재가 **UDP 514**·`$UDPServerRun`·`*.* @호스트`를 묻는 이유는, **전통적 BSD syslog(RFC 3164) 원격 전달이 비연결 UDP에 가깝게 굳어졌기 때문**이다. **신뢰 전달**은 RFC 3195(TCP+BEEP) 등으로 논의되고, `rsyslog`는 **`imudp`(UDP 514)·`imtcp`(TCP 514)** 로 프로토콜을 고를 수 있다 ([[cs/information-security/01.system-security/03.linux-basic|03 §10-1]] 참고). |
| **참고 (`rsyslog`)** | 전달 구문으로 **단일 `@`** 와 **이중 `@@`** 를 쓰는 관례가 있어(버전·모듈에 따라) **UDP vs TCP** 구분에 쓰이기도 한다. **이 문서·시험 함정은 `@` 한 개·UDP 514 절차에 맞추면 된다.** |
...
> **`*.*`** = `facility.priority` 의 *모든 facility · 모든 priority*. **`*.* @[호스트]`** 에서 앞쪽 `*` 쌍은 “무엇을 보낼지”, **`@`** 는 (이 문맥에서는) **원격으로 전달**하기 위한 **rsyslog 전송 액션** 표기이다.
