...
| 권한 | 약어 | 8진수 | 의미 |
|---|---|---|---|
| **SUID** | Set-UID (`s`) | **4** | 실행 중 *실행 파일 소유자 권한*으로 자원 접근 |
| **SGID** | Set-GID (`s`) | **2** | 실행 중 *실행 파일 소유 그룹 권한*으로 자원 접근 |
...
| 식별자 | 풀이 | 의미 |
|---|---|---|
| **RUID/RGID** | **R**eal | 프로세스를 *실행시킨 사용자* UID/GID |
| **EUID/EGID** | **E**ffective | *접근 권한 판단에 쓰이는* UID/GID |
...
| **`cps`** | *초당 최대 연결* (예: `50 10` = 초당 50, 초과 시 10초 제한) |
