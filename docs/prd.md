...
### 문제 정의
- YouTube 추출기(`~/dev/personal/007_youtube-script`)는 영상 → 자막/STT → **canonical JSON + MD** 를 산출하지만(`domain.py:85-109`), cs-study 위키로 합류시키는 **ingest 실행 계층이 부재**하다(`scripts/ingest.py` 없음 — 실측 `ls` 0건; AGENTS.md:46-57 은 ingest 순서만 규정).
...
| FR-4 | canonical JSON → raw frontmatter **필드 매핑**을 결정적으로 수행한다(§매핑표). | `title/source_url/source_date/source_type/last_verified/ingested_date/tier` 전부 채워짐. `tier: raw`, `source_type: video`. | Must |
...
| NFR-5 | **자기 코드 호출 깊이**: 신규 importer 직렬 체인 함수 ≤5(edge ≤4). | arch-layer-depth 임계 내. |
...
- **제외 (2차 라운드 이연)**:
  ...
  - software-engineering 등 video 대응 wiki 도메인 신설(현 wiki는 llm-* 6개뿐).
...
| 엣지 케이스 | `video.title`/`channel` null | title null→video.id fallback. 기타 null은 빈 값 허용(raw 필수 아님) | FR-4 매핑표 |
