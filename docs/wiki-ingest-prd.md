...
| 지식 합성 | `001_cs-study` synthesize | 명시적 artifact manifest 목록 | 검증 대상 draft | raw 수정·LLM 직접 canonical write |
...
| FR-KP-006 | 합성 입력은 manifest 경로의 명시적 목록이다 | implicit latest 선택과 wiki 재-ingest가 거부된다 |
| FR-KP-007 | LLM 결과는 semantic draft일 뿐 canonical write 권한이 없다 | path·frontmatter·derived field·rendered Markdown·write operation 입력이 거부되고 Python이 재계산한다 |
...
| FR-KP-015 | draft와 active lifecycle을 분리한다 | review 승인 없는 draft→active 전이가 실패한다 |
| FR-KP-016 | 일반 lifecycle·page command의 한 apply는 canonical page를 최대 한 개만 변경한다 | apply write-set의 canonical Markdown 수가 0 또는 1이며, NFR-KP-015의 사용자 승인 전역 schema migration은 별도 resolved-plan·backup·full-tree transaction 규칙을 따른다 |
...
| FR-KP-022 | 페이지 이동은 ID를 보존한다 | move 전후 `id` 동일, inbound derived view 재생성 후 broken link 0건이다 |
...
| NFR-KP-009 | 안전성 | validate-before-write, single-page atomic replace, optimistic base digest를 적용한다 |
...
3. synthesize가 한 개 이상의 manifest를 입력받아 draft 한 개를 계획한다.
4. checker가 schema, IDs, source, claims, relations, link, collision을 검증한다.
5. 사람이 claim grounding과 collection 순서를 검토한다.
6. promote가 base digest와 target collision을 확인하고 content bytes를 바꾸지 않은 채 canonical page 한 개를 active path로 atomic rename한다.
7. materializer가 navigation과 template을 재생성한다.
8. CI가 full check와 regeneration diff를 실행한다.
...
| AC-KP-009 | apply 실패 주입 후 canonical page의 이전 bytes가 보존된다 |
...
| AC-KP-013 | primary claim마다 실재 evidence와 review verdict가 있다 |
