...
| 항목 | 내용 |
|---|---|
| 갱신 대상 task | P2-T1~P2-T11 등록 및 순서 1–6b baseline commit 선행 결함과 순서 7–12 추적 |
| 신규 등록 후보 | 변경 626건 분류·검증, 절대경로 비식별화, runtime artifact ignore guard, baseline commit, 순서 7–12 |
| 범위 밖 | 현재 turn의 source·wiki·raw 수정, staging 변경, commit, push, migration apply·restore·recover |
| 검증 기준 | 상태·분류 enum, 의미 중복 없음, 직접 선행 단방향·cycle 0, commit·migration apply 별도 사용자 승인 유지 |
...
| P1-T3 | [x] | [검증] | 승인된 wiki 이동 8건이 만드는 관찰 external reference 676회를 owner별로 분류하고 active path만 바꾸는 no-write cascade plan과 전체 diff를 생성해 `sourceRef` line·excerpt·status 보존 및 hidden-inclusive active stale scanner 0건(명시적 제외·historical 별도 보고)을 검증한 뒤 actual apply 승인 자료를 제출한다 | 없음 | `_meta/knowledge-migration-resolution.json`, `docs/wiki-ingest-review.md`, `projects/info-sec-engineer-practice/data/question-packs/`, `projects/info-sec-engineer-practice/practice-data.js`, `.claude/resume_prompt.md` | canonical question-pack 329회 path 변경 계획, generated JS 재생성 계획, project docs·system refs cascade diff, resolved plan·preview·backup digest와 별도 사용자 apply gate |
...
| P2-T5 | [ ] | [구현] | 순서 7 synthesize·promote·collection·move leaf command를 TDD로 구현한다 | P2-T4 | 6b live target, SemanticPlan·leaf write-set·stale base digest 계약 | one-page write-set·stale base digest 테스트와 독립 commit | |
| P2-T6 | [ ] | [구현] | 순서 8 materializer를 구현하고 index·overview·template·Bases를 generated view로 전환한다 | P2-T5 | canonical pages·registry·schema | two-run tree hash·active coverage 100%와 독립 commit | |
| P2-T7 | [ ] | [검증] | 순서 9 legacy structure-rule scope·log/backlink/provenance 제거를 no-write plan과 derived parity로 검증한다 | P2-T6 | legacy rules·log·index 선언 | 승인용 removal plan·repository grep·derived parity report | |
| P2-T8 | [ ] | [운영] | P2-T7 no-write plan의 별도 사용자 승인 후 순서 9 migration apply를 실행한다 | P2-T7 | 승인 digest·backup·removal plan | terminal journal·post-apply 검증·독립 commit | |
