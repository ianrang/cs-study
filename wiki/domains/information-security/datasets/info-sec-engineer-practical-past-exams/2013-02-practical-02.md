...
| 15 | essay | 다음은 IIS 웹 로그 분석 결과와 관련된 공격에 관한 물음에 답하시오. | (1) SQL Injection (2) SQL 쿼리문에서 '--로 인해 패스워드 검증 조건이 주석 처리되어 무력화되므로, 올바른 패스워드 입력 없이 admin 계정으로 인증이 우회되어 로그인이 가능해진다. (3) 모든 입력값을 검증하여 사용자가 입력한 값에 ', or, and, --, " 등 SQL 특수문자가 포함되지 않도록 필터링한다. | source-derived from Information Security Tistory; answer block present |
