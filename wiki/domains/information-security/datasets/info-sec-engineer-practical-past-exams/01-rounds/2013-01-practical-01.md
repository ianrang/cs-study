...
| 6 | short | SIEM 주요 기능의 빈칸을 채우시오. (A): 관제 대상 시스템 에이전트가 SNMP·syslog 등으로 로그를 저장하는 과정. (B): 이벤트 발생 누적 횟수 등 유사 정보를 기준으로 그룹화해 하나의 정보로 취합하는 과정. (C): 다양한 로그 표현 형식을 표준 포맷으로 변환하는 과정. (D): 표준화된 로그의 타임스탬프, IP 주소, 이벤트 구성 규칙을 기준으로 여러 로그 간 관계를 분석하는 과정. | A : 로그 수집 B : 로그 분류 C : 로그 변환 D : 로그 분석 | source-derived from Information Security Tistory; prompt list restored |
