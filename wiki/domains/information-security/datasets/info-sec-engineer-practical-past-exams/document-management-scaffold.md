...
| 문서 규칙 확인 | `document-architecture.md` | 이 문서 | 분석 본문을 관리 문서에 복제하지 않는다. |
...
| 학습 실행 | `study-strategy-2026-02.md` | `predicted-practical-questions-2026-02.md` | 예상문제를 실제 출제 보장처럼 표현하지 않는다. |
...
| 그룹 | 문서 | 응집 기준 | 주 변경 이유 |
|---|---|---|---|
| Management | `document-architecture.md`, `document-management-scaffold.md`, `analysis-roadmap-todo.md` | 문서 운영 규칙과 작업 상태 | 문서군의 역할, 절차, 작업 순서가 바뀔 때 |
| Navigation | `index.md` | 회차 복원 파일과 전체 커버리지 탐색 | 회차 파일이 추가·이동·상태 변경될 때 |
| Round Reconstruction | `2013-01-practical-01.md` through `2025-04-practical-30.md` | 회차별 문항·답안·출처·확신도 | 기출 복원 원천이 보강되거나 문항 오류가 발견될 때 |
...
| Study Output | `study-strategy-2026-02.md`, `predicted-practical-questions-2026-02.md`, `prediction-validation-report.md` | 학습 전략, 예상문제, 예상문제 검증 | 시험 대비 전략 또는 예상문제 세트가 바뀔 때 |
| Prompt Quality | `prompt-completeness-cross-verify-report.md` | 문항 지시문 완전성 검증 | 설명 누락, 과압축, known-limited 항목이 발견·해소될 때 |
...
## 물리 스캐폴딩 정책
- 현재 파일은 같은 디렉터리에 둔다.
- 물리 디렉터리 분리는 링크·frontmatter·인덱스 마이그레이션을 동반하는 별도 작업으로만 한다.
