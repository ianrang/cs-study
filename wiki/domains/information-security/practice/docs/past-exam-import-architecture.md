...
`datasets/info-sec-engineer-practical-past-exams/01-rounds/*.md`의 회차별 복원 문항을 수정·복제하지 않고 Practice에서 풀이한다.

- 회차 MD는 문항·답·복원 근거의 SSOT이며 읽기 전용이다.
- `practice/data/question-packs/*.json`은 학습용 구조화 문항의 SSOT로 유지한다.
- 변환 결과인 `practice-data.js`만 브라우저에 전달한다.
...
```text
round MD (read-only SSOT) ─┐
                           ├─> past_exam_converter.py ─> practice-data.js ─> app.js
...
- LocalStorage에는 `past-exam:<round-item-id>` 진행 상태만 저장한다. 회차 MD와는 독립된 별도 키를 사용한다.
...
5. 표의 escape된 파이프(`\|`)는 셀 내용으로 보존한다. 열 수가 맞지 않거나 번호가 중복·비연속이면 빌드를 실패시킨다.
6. 모든 기출 레코드는 `source-derived`로 표시한다. 변환기는 공식 원문 여부를 승격하지 않는다.
7. 회차 MD 변경으로 `practice-data.js`가 달라지면 `--check`가 stale 상태로 실패해야 한다.
