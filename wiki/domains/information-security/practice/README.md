...
현재 학습한 1장 P1 시스템 보안, 2장 네트워크·방화벽, 3장 P1 서비스 보안설정, 5장 P1 관리체계·위험관리·업무연속성·침해사고·포렌식·개인정보 기초 범위를 반복하기 위한 정적 브라우저 앱이다. 실제 방화벽·네트워크 장비를 조작하지 않으며, 기존 `../labs/`의 offline 실습과 별도로 동작한다.
...
1. 좌측에서 학습 경로·원본 장/절·세부 주제·단계를 고른다. 전체·주제 기준 기본 순서는 선수 문항을 의존 문항보다 먼저 보이게 하며, 단계 필터를 선택하면 해당 단계만 본다.
