...
**시간이 부족하면 제외 가능한 범위**: 2.8 전체, TLS 1.3의 0-RTT·DTLS 세부, SNMPv3 내부 필드, IPv4 Options, Land·Ping of Death의 세부 동작은 마지막 회독으로 미룬다. 다만 P3도 완전히 삭제하지 않은 이유는 단답형에서 저빈도 용어가 다시 나올 수 있기 때문이다.
...
| P1 | IDS/IPS·Snort·관제 | 반복 개념군 52건, 단답과 서술/실무를 오감 | 분류·오탐/미탐 → 룰 해석 → 배치·대응 |
...
- IPv4는 32비트다. `/n`은 앞에서부터 네트워크 비트 수이며, 호스트 비트 수는 `32-n`이다.
- 주소 수는 `2^(호스트 비트 수)`. 일반적인 IPv4 서브넷의 사용 가능한 호스트 수는 네트워크·브로드캐스트 주소를 제외한 `2^h-2`다. `/31`·`/32` 같은 예외는 문제 조건을 따른다.
- 예: `200.100.50.25/26`의 마스크는 `255.255.255.192`, 2진수로 `11111111.11111111.11111111.11000000`, 블록 크기는 64다. 해당 네트워크는 `200.100.50.0`, 브로드캐스트는 `200.100.50.63`, 일반 호스트 범위는 `.1~.62`다.
...
- RIP는 거리 벡터·hop count(최대 15), OSPF는 링크 상태·cost와 SPF 계산, EIGRP는 DUAL을 사용하는 Cisco 계열의 고급 거리 벡터 성격을 기억한다.
- 라우터는 목적지와 일치하는 경로 중 prefix가 가장 긴 **Longest Prefix Match**를 우선한다. 일치 경로가 없으면 default route `0.0.0.0/0`를 사용하고, 그것도 없으면 폐기한다.
- Host route는 한 주소를 지정하는 `/32`, network route는 특정 prefix 대역, default route는 나머지 목적지를 처리하는 `/0`이다.
- 라우팅 테이블은 `목적지 network/prefix → next-hop gateway 또는 직접 연결 → 출력 interface → metric` 순으로 읽는다. `ip route`와 `route print`에서 목적지까지 실제로 어떤 gateway와 interface를 거치는지 확인한다.
- 같은 목적지 후보에서는 라우팅 프로토콜의 신뢰도(Administrative Distance 등)와 metric을 이용해 최종 경로를 선택한다. 정확한 기본값 전체보다 **prefix → 경로 출처 → metric**의 판독 순서를 우선한다.
- 위조 경로 광고는 트래픽 우회·도청·블랙홀을 만들 수 있다. 라우팅 프로토콜 인증, 신뢰 인터페이스 제한, passive-interface, 경로 필터링, 관리망 분리, 로그 모니터링을 적용한다.
- `no ip source-route`는 패킷 송신자가 경로를 지정하는 IP source routing을 비활성화하는 대표적 방어다.
...
| 공격 | 우선순위 | 원리·판단 근거 | 핵심 대응 |
|---|---|---|---|
| SYN Flooding | P1 | SYN만 대량 전송하여 half-open backlog 고갈 | SYN cookie, backlog·타임아웃 조정, rate limit, Anti-DDoS |
| Smurf | P1 | 피해자 IP로 위조한 ICMP Echo를 directed broadcast에 전송해 다수 Reply 반사 | directed broadcast 차단, ICMP rate limit, 출발지 검증 |
| DNS/NTP 증폭 DRDoS | P1 | 피해자 IP로 UDP 요청을 위조하고 작은 요청 대비 큰 응답을 반사 | 출발지 검증, open resolver·불필요 NTP 기능 제한, RRL·rate limit |
| Slow HTTP | P1 | 연결·요청·응답을 매우 느리게 유지해 자원 고갈 | 연결·헤더·본문 타임아웃, 동시 연결 제한, reverse proxy/WAF |
| Teardrop | P2 | 중첩·비정상 fragment offset으로 재조립 오류 유발 | 패치, 비정상 단편 필터·재조립 검사 |
| Land | P2 | 출발지와 목적지 IP·포트를 피해자와 같게 위조 | 비정상 동일 주소 필터, 패치 |
| Ping of Death | P2 | 허용 크기를 넘는 ICMP/IP 재조립 유도 | 패치, 비정상·과대 단편 차단 |
...
- NTP는 불필요한 외부 질의를 제한하고, 레거시 `monlist`가 제거된 지원 버전으로 갱신하며, 접근제어와 rate limit을 적용한다. 특정 버전 번호·지시자는 문제에 제시된 환경을 우선한다.
...
- [ ] SYN Flooding, Smurf, DNS/NTP 증폭, Slow HTTP의 원리와 대응을 쓴다.
...
2. SYN scan에서 열린 포트와 닫힌 포트는 각각 무엇을 응답하는가?
3. DDoS와 DRDoS의 차이는 무엇인가?
4. Smurf와 DNS/NTP 증폭은 출발지 위조를 어떻게 이용하는가?
5. VLAN hopping 두 방식과 각각의 대응은 무엇인가?
6. `iptables`의 INPUT·OUTPUT·FORWARD는 어떻게 다른가?
7. 방화벽 룰을 어떤 순서로 해석하는가?
8. IDS/IPS, HIDS/NIDS, 오용/이상 탐지는 어떻게 다른가?
9. False Positive와 False Negative 중 실제 공격을 놓친 것은 무엇인가?
10. Snort 룰의 header와 option을 어떻게 읽는가?
11. SNMPv3 authPriv를 쓰는 이유와 보완책 네 가지는 무엇인가?
12. AH·ESP·IKE·SA의 역할은 무엇인가?
13. IPsec 전송 모드와 터널 모드는 무엇을 보호하는가?
14. TLS 인증서 공개키·임시 키 교환·대칭 세션키의 역할은 무엇인가?
15. 라우팅 테이블에서 Longest Prefix Match와 default route는 어떻게 적용되는가?
16. DNS 질의는 stub resolver에서 Authoritative DNS까지 어떻게 진행되는가?
17. 패킷 한 줄을 보고 공격을 단정하면 안 되는 이유는 무엇인가?
